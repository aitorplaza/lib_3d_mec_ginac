...
if [  ]; then 
	gcc  $I3AL $Q $DDL $INV $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $COORD_Q $DYN_DDL $DYN_DDQ $I3AL_Q one_step_i3al_q.c -lm
	./$MAIN $integration $delta_t $name;
fi
#####################################################################################################################################################
name="state_lin_alg_i3al_z_inv.dat"
if [  ]; then 
	gcc  $I3AL $Z $INV $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $COORD_Z $DYN_DDL $I3AL_Z $I3AL_Q one_step_i3al_z.c -lm
	./$MAIN $integration $delta_t $name;
fi
#####################################################################################################################################################
