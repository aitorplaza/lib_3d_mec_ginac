...
// *****************************************************************************************************************	
    ...
    cout << "********************************************************" << endl;
    cout << "*             Train                                    *" << endl;
    cout << "*             COMPLETE v8.1 SPLINES & KALKER           *" << endl;
    cout << "*             RAIL WAY WITH SPLINES                    *" << endl;    
    cout << "********************************************************" << endl;
...
// ************************************************************************************************
    ...
    ex sR_RFR = atomize_ex (uR_RFR - lR_RFR);
    ex splineR_RFR =   atomize_ex (dR_RFR + sR_RFR * ( cR_RFR + sR_RFR * (bR_RFR + sR_RFR * aR_RFR ) ));
    Vector3D fR_RFR = *sys.new_Vector3D("fR_RFR",0.0,uR_RFR,splineR_RFR,"B_RAIL");
    
    // spline rail
    symbol_numeric RFRas = *sys.new_Parameter("RFRas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric RFRbs = *sys.new_Parameter("RFRbs",0.0);
    symbol_numeric RFRcs = *sys.new_Parameter("RFRcs",0.0);
    symbol_numeric RFRds = *sys.new_Parameter("RFRds",0.0);
    symbol_numeric RFRes = *sys.new_Parameter("RFRes",0.0);
    symbol_numeric RFRfs = *sys.new_Parameter("RFRfs",0.0);    
    symbol_numeric RFRstas = *sys.new_Parameter("RFRstas",0.0);
    symbol_numeric RFRends = *sys.new_Parameter("RFRends",10.0);
    ex slR_RFR   = atomize_ex ((xR_RFR - RFRstas)/(RFRends-RFRstas));
    
    ex spS_RFR   = atomize_ex (RFRfs + slR_RFR * ( RFRes + slR_RFR * (RFRds + slR_RFR * ( RFRcs + slR_RFR * (RFRbs + slR_RFR * RFRas ) ) )) );
    //ex spS_RFR   = 0;
    ex dspS_RFRx = sys.diff ( xR_RFR  , xR_RFR);
    ex dspS_RFRy = sys.diff ( spS_RFR , xR_RFR);
    ex dspS_RFR_mod = atomize_ex(sqrt(dspS_RFRx*dspS_RFRx + dspS_RFRy*dspS_RFRy));
    Vector3D pR_RFR  = *sys.new_Vector3D("pR_RFR",xR_RFR + (L_RAIL/2.0) * dspS_RFRy / dspS_RFR_mod , spS_RFR - (L_RAIL/2.0) *dspS_RFRx/dspS_RFR_mod ,0.0,"B_RAIL");
    ...
    ex sR_RFL = atomize_ex (uR_RFL - lR_RFL);
    ex splineR_RFL =   atomize_ex (dR_RFL + sR_RFL * ( cR_RFL + sR_RFL * (bR_RFL + sR_RFL * aR_RFL)) );
    Vector3D fR_RFL = *sys.new_Vector3D("fR_RFL",0.0,uR_RFL,splineR_RFL,"B_RAIL");
    
    symbol_numeric RFLas = *sys.new_Parameter("RFLas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric RFLbs = *sys.new_Parameter("RFLbs",0.0);
    symbol_numeric RFLcs = *sys.new_Parameter("RFLcs",0.0);
    symbol_numeric RFLds = *sys.new_Parameter("RFLds",0.0);
    symbol_numeric RFLes = *sys.new_Parameter("RFLes",0.0);
    symbol_numeric RFLfs = *sys.new_Parameter("RFLfs",0.0);    
    symbol_numeric RFLstas = *sys.new_Parameter("RFLstas",0.0);
    symbol_numeric RFLends = *sys.new_Parameter("RFLends",10.0);    
    ex slR_RFL   = atomize_ex ((xR_RFL - RFLstas)/(RFLends-RFLstas));
    
    ex spS_RFL   = atomize_ex (RFLfs + slR_RFL * ( RFLes + slR_RFL * (RFLds + slR_RFL * ( RFLcs + slR_RFL * (RFLbs + slR_RFL * RFLas ) ) )) );
    //ex spS_RFL   = 0.0;
    ex dspS_RFLx = sys.diff ( xR_RFL  , xR_RFL);
    ex dspS_RFLy = sys.diff ( spS_RFL , xR_RFL);
    ex dspS_RFL_mod = atomize_ex(sqrt(dspS_RFLx*dspS_RFLx + dspS_RFLy*dspS_RFLy));
    
    Vector3D pR_RFL  = *sys.new_Vector3D("pR_RFL",xR_RFL - (L_RAIL/2.0) * dspS_RFLy / dspS_RFL_mod , spS_RFL + (L_RAIL/2.0) *dspS_RFLx/dspS_RFL_mod ,0.0,"B_RAIL");
...
cout << " RRR" <<endl;
    ...
    ex sR_RRR = atomize_ex (uR_RRR - lR_RRR);
    ex splineR_RRR =   atomize_ex (dR_RRR + sR_RRR * ( cR_RRR + sR_RRR * (bR_RRR + sR_RRR * aR_RRR ) ));
    Vector3D fR_RRR = *sys.new_Vector3D("fR_RRR",0.0,uR_RRR,splineR_RRR,"B_RAIL");
    
    // spline rail
    symbol_numeric RRRas = *sys.new_Parameter("RRRas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric RRRbs = *sys.new_Parameter("RRRbs",0.0);
    symbol_numeric RRRcs = *sys.new_Parameter("RRRcs",0.0);
    symbol_numeric RRRds = *sys.new_Parameter("RRRds",0.0);
    symbol_numeric RRRes = *sys.new_Parameter("RRRes",0.0);
    symbol_numeric RRRfs = *sys.new_Parameter("RRRfs",0.0);    
    symbol_numeric RRRstas = *sys.new_Parameter("RRRstas",0.0);
    symbol_numeric RRRends = *sys.new_Parameter("RRRends",10.0);
    ex slR_RRR   = atomize_ex ((xR_RRR - RRRstas)/(RRRends-RRRstas));
    
    ex spS_RRR   = atomize_ex (RRRfs + slR_RRR * ( RRRes + slR_RRR * (RRRds + slR_RRR * ( RRRcs + slR_RRR * (RRRbs + slR_RRR * RRRas ) ) )) );
    //ex spS_RRR   =  0.0;
    ex dspS_RRRx = sys.diff ( xR_RRR  , xR_RRR);
    ex dspS_RRRy = sys.diff ( spS_RRR , xR_RRR);
    ex dspS_RRR_mod = atomize_ex(sqrt(dspS_RRRx*dspS_RRRx + dspS_RRRy*dspS_RRRy));
    
    Vector3D pR_RRR  = *sys.new_Vector3D("pR_RRR",xR_RRR + (L_RAIL/2.0) * dspS_RRRy / dspS_RRR_mod , spS_RRR - (L_RAIL/2.0) *dspS_RRRx/dspS_RRR_mod ,0.0,"B_RAIL");
...
cout << " RRL" <<endl;
    ...
    ex sR_RRL = atomize_ex (uR_RRL - lR_RRL);
    ex splineR_RRL =   atomize_ex (dR_RRL + sR_RRL * ( cR_RRL + sR_RRL * (bR_RRL + sR_RRL * aR_RRL ) ));
    Vector3D fR_RRL = *sys.new_Vector3D("fR_RRL",0.0,uR_RRL,splineR_RRL,"B_RAIL");
    
    // spline rail
    symbol_numeric RRLas = *sys.new_Parameter("RRLas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric RRLbs = *sys.new_Parameter("RRLbs",0.0);
    symbol_numeric RRLcs = *sys.new_Parameter("RRLcs",0.0);
    symbol_numeric RRLds = *sys.new_Parameter("RRLds",0.0);
    symbol_numeric RRLes = *sys.new_Parameter("RRLes",0.0);
    symbol_numeric RRLfs = *sys.new_Parameter("RRLfs",0.0);    
    symbol_numeric RRLstas = *sys.new_Parameter("RRLstas",0.0);
    symbol_numeric RRLends = *sys.new_Parameter("RRLends",10.0);    
    ex slR_RRL   = atomize_ex ((xR_RRL - RRLstas)/(RRLends-RRLstas));
    
    ex spS_RRL   = atomize_ex (RRLfs + slR_RRL * ( RRLes + slR_RRL * (RRLds + slR_RRL * ( RRLcs + slR_RRL * (RRLbs + slR_RRL * RRLas ) ) )) );
    //ex spS_RRL   = 0.0;
    ex dspS_RRLx = sys.diff ( xR_RRL  , xR_RRL);
    ex dspS_RRLy = sys.diff ( spS_RRL , xR_RRL);
    ex dspS_RRL_mod = atomize_ex(sqrt(dspS_RRLx*dspS_RRLx + dspS_RRLy*dspS_RRLy));

    Vector3D pR_RRL  = *sys.new_Vector3D("pR_RRL",xR_RRL - (L_RAIL/2.0) * dspS_RRLy / dspS_RRL_mod , spS_RRL + (L_RAIL/2.0) *dspS_RRLx/dspS_RRL_mod ,0.0,"B_RAIL");
...
cout << " FFR" <<endl;   
    ...
    ex sR_FFR = atomize_ex (uR_FFR - lR_FFR);
    ex splineR_FFR =   atomize_ex (dR_FFR + sR_FFR * ( cR_FFR + sR_FFR * (bR_FFR + sR_FFR * aR_FFR ) ));
    Vector3D fR_FFR = *sys.new_Vector3D("fR_FFR",0.0,uR_FFR,splineR_FFR,"B_RAIL");
    
    // spline rail
    symbol_numeric FFRas = *sys.new_Parameter("FFRas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric FFRbs = *sys.new_Parameter("FFRbs",0.0);
    symbol_numeric FFRcs = *sys.new_Parameter("FFRcs",0.0);
    symbol_numeric FFRds = *sys.new_Parameter("FFRds",0.0);
    symbol_numeric FFRes = *sys.new_Parameter("FFRes",0.0);
    symbol_numeric FFRfs = *sys.new_Parameter("FFRfs",0.0);    
    symbol_numeric FFRstas = *sys.new_Parameter("FFRstas",0.0);
    symbol_numeric FFRends = *sys.new_Parameter("FFRends",10.0);
    ex slR_FFR   = atomize_ex ((xR_FFR - FFRstas)/(FFRends-FFRstas));

    ex spS_FFR   = atomize_ex (FFRfs + slR_FFR * ( FFRes + slR_FFR * (FFRds + slR_FFR * ( FFRcs + slR_FFR * (FFRbs + slR_FFR * FFRas ) ) )) );
    //ex spS_FFR   = 0.0;
    ex dspS_FFRx = sys.diff ( xR_FFR  , xR_FFR);
    ex dspS_FFRy = sys.diff ( spS_FFR , xR_FFR);
    ex dspS_FFR_mod = atomize_ex(sqrt(dspS_FFRx*dspS_FFRx + dspS_FFRy*dspS_FFRy));
    
    Vector3D pR_FFR  = *sys.new_Vector3D("pR_FFR",xR_FFR + (L_RAIL/2.0) * dspS_FFRy / dspS_FFR_mod , spS_FFR - (L_RAIL/2.0) *dspS_FFRx/dspS_FFR_mod ,0.0,"B_RAIL");
...
cout << " FFL" <<endl;
    ...
    ex sR_FFL = atomize_ex (uR_FFL - lR_FFL);
    ex splineR_FFL =   atomize_ex (dR_FFL + sR_FFL * ( cR_FFL + sR_FFL * (bR_FFL + sR_FFL * aR_FFL ) ));
    Vector3D fR_FFL = *sys.new_Vector3D("fR_FFL",0.0,uR_FFL,splineR_FFL,"B_RAIL");
    
    // spline rail
    symbol_numeric FFLas = *sys.new_Parameter("FFLas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric FFLbs = *sys.new_Parameter("FFLbs",0.0);
    symbol_numeric FFLcs = *sys.new_Parameter("FFLcs",0.0);
    symbol_numeric FFLds = *sys.new_Parameter("FFLds",0.0);
    symbol_numeric FFLes = *sys.new_Parameter("FFLes",0.0);
    symbol_numeric FFLfs = *sys.new_Parameter("FFLfs",0.0);    
    symbol_numeric FFLstas = *sys.new_Parameter("FFLstas",0.0);
    symbol_numeric FFLends = *sys.new_Parameter("FFLends",10.0);
    ex slR_FFL   = atomize_ex ((xR_FFL - FFLstas)/(FFLends-FFLstas));

    ex spS_FFL   = atomize_ex (FFLfs + slR_FFL * ( FFLes + slR_FFL * (FFLds + slR_FFL * ( FFLcs + slR_FFL * (FFLbs + slR_FFL * FFLas ) ) )) );
    //ex spS_FFL   = 0.0;
    ex dspS_FFLx = sys.diff ( xR_FFL  , xR_FFL);
    ex dspS_FFLy = sys.diff ( spS_FFL , xR_FFL);
    ex dspS_FFL_mod = atomize_ex(sqrt(dspS_FFLx*dspS_FFLx + dspS_FFLy*dspS_FFLy));

    Vector3D pR_FFL  = *sys.new_Vector3D("pR_FFL",xR_FFL - (L_RAIL/2.0) * dspS_FFLy / dspS_FFL_mod , spS_FFL + (L_RAIL/2.0) *dspS_FFLx/dspS_FFL_mod ,0.0,"B_RAIL");
...
cout << " FRR " <<endl;
    ...
    ex sR_FRR = atomize_ex (uR_FRR - lR_FRR);
    ex splineR_FRR =   atomize_ex (dR_FRR + sR_FRR * ( cR_FRR + sR_FRR * (bR_FRR + sR_FRR * aR_FRR ) ));
    Vector3D fR_FRR = *sys.new_Vector3D("fR_FRR",0.0,uR_FRR,splineR_FRR,"B_RAIL");
    
    // spline rail
    symbol_numeric FRRas = *sys.new_Parameter("FRRas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric FRRbs = *sys.new_Parameter("FRRbs",0.0);
    symbol_numeric FRRcs = *sys.new_Parameter("FRRcs",0.0);
    symbol_numeric FRRds = *sys.new_Parameter("FRRds",0.0);
    symbol_numeric FRRes = *sys.new_Parameter("FRRes",0.0);
    symbol_numeric FRRfs = *sys.new_Parameter("FRRfs",0.0);    
    symbol_numeric FRRstas = *sys.new_Parameter("FRRstas",0.0);
    symbol_numeric FRRends = *sys.new_Parameter("FRRends",10.0);
    ex slR_FRR   = atomize_ex ((xR_FRR - FRRstas)/(FRRends-FRRstas));
    
    ex spS_FRR   = atomize_ex (FRRfs + slR_FRR * ( FRRes + slR_FRR * (FRRds + slR_FRR * ( FRRcs + slR_FRR * (FRRbs + slR_FRR * FRRas ) ) )) );
    //ex spS_FRR   = 0.0;
    ex dspS_FRRx = sys.diff ( xR_FRR  , xR_FRR);
    ex dspS_FRRy = sys.diff ( spS_FRR , xR_FRR);
    ex dspS_FRR_mod = atomize_ex(sqrt(dspS_FRRx*dspS_FRRx + dspS_FRRy*dspS_FRRy));

    Vector3D pR_FRR  = *sys.new_Vector3D("pR_FRR",xR_FRR + (L_RAIL/2.0) * dspS_FRRy / dspS_FRR_mod , spS_FRR - (L_RAIL/2.0) *dspS_FRRx/dspS_FRR_mod ,0.0,"B_RAIL");
    ...
    ex sR_FRL = atomize_ex (uR_FRL - lR_FRL);
    ex splineR_FRL =   atomize_ex (dR_FRL + sR_FRL * ( cR_FRL + sR_FRL * (bR_FRL + sR_FRL * aR_FRL ) ));
    Vector3D fR_FRL = *sys.new_Vector3D("fR_FRL",0.0,uR_FRL,splineR_FRL,"B_RAIL");
    
    // spline rail
    symbol_numeric FRLas = *sys.new_Parameter("FRLas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric FRLbs = *sys.new_Parameter("FRLbs",0.0);
    symbol_numeric FRLcs = *sys.new_Parameter("FRLcs",0.0);
    symbol_numeric FRLds = *sys.new_Parameter("FRLds",0.0);
    symbol_numeric FRLes = *sys.new_Parameter("FRLes",0.0);
    symbol_numeric FRLfs = *sys.new_Parameter("FRLfs",0.0);    
    symbol_numeric FRLstas = *sys.new_Parameter("FRLstas",0.0);
    symbol_numeric FRLends = *sys.new_Parameter("FRLends",10.0);
    ex slR_FRL   = atomize_ex ((xR_FRL - FRLstas)/(FRLends-FRLstas));
    
    ex spS_FRL   = atomize_ex (FRLfs + slR_FRL * ( FRLes + slR_FRL * (FRLds + slR_FRL * ( FRLcs + slR_FRL * (FRLbs + slR_FRL * FRLas ) ) )) );
    //ex spS_FRL   =0.0;
    ex dspS_FRLx = sys.diff ( xR_FRL  , xR_FRL);
    ex dspS_FRLy = sys.diff ( spS_FRL , xR_FRL);
    ex dspS_FRL_mod = atomize_ex(sqrt(dspS_FRLx*dspS_FRLx + dspS_FRLy*dspS_FRLy));

    Vector3D pR_FRL  = *sys.new_Vector3D("pR_FRL",xR_FRL - (L_RAIL/2.0) * dspS_FRLy / dspS_FRL_mod , spS_FRL + (L_RAIL/2.0) *dspS_FRLx/dspS_FRL_mod ,0.0,"B_RAIL");
