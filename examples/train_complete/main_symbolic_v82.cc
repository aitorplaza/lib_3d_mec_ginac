...
// *****************************************************************************************************************	
    ...
    cout << "********************************************************" << endl;
    cout << "*             Train                                    *" << endl;
    cout << "*             COMPLETE v8.2 SPLINES & KALKER           *" << endl;
    cout << "*             RAIL WAY WITH CUBIC SPLINES              *" << endl;    
    cout << "********************************************************" << endl;
...
// ************************************************************************************************************************************************************************************************
    ...
    // RAXBFL BASE   
    sys.new_Base("B_RAXBFL","B_RWHSF_a",0,1,0,0);
    // RAXBFR BASE   
    sys.new_Base("B_RAXBFR","B_RWHSF_a",0,1,0,0);
    // RAXBRL BASE   
    sys.new_Base("B_RAXBRL","B_RWHSR_a",0,1,0,0);
    // RAXBRR BASE   
    sys.new_Base("B_RAXBRR","B_RWHSR_a",0,1,0,0);

    // FAXBFL BASE   
    sys.new_Base("B_FAXBFL","B_FWHSF_a",0,1,0,0);
    // FAXBFR BASE   
    sys.new_Base("B_FAXBFR","B_FWHSF_a",0,1,0,0);
    // FAXBRL BASE   
    sys.new_Base("B_FAXBRL","B_FWHSR_a",0,1,0,0);
    // FAXBRR BASE   
    sys.new_Base("B_FAXBRR","B_FWHSR_a",0,1,0,0);
...
// ************************************************************************************************
    ...
    Vector3D S1RBOG_ORMOTF  = *sys.new_Vector3D("S1RBOG_ORMOTF",0,0,0,"B_RBOG");
    Vector3D S2RBOG_ORMOTR  = *sys.new_Vector3D("S2RBOG_ORMOTR",0,0,0,"B_RBOG");
...
// ************************************************************************************************
    ...
    symbol_numeric sR_RFR = *sys.new_AuxCoordinate("sR_RFR","dsR_RFR","ddsR_RFR", -3.65595 ,0.0,0.0);
    ...
    ex slR_RFRp = atomize_ex (uR_RFR - lR_RFR);
    ex splineR_RFR =   atomize_ex (dR_RFR + slR_RFRp * ( cR_RFR + slR_RFRp * (bR_RFR + slR_RFRp * aR_RFR ) ));
    Vector3D fR_RFR = *sys.new_Vector3D("fR_RFR",0.0,uR_RFR,splineR_RFR,"B_RAIL");
    
    // spline rail
    symbol_numeric xRFRas = *sys.new_Parameter("xRFRas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric xRFRbs = *sys.new_Parameter("xRFRbs",0.0);
    symbol_numeric xRFRcs = *sys.new_Parameter("xRFRcs",1.0);
    symbol_numeric xRFRds = *sys.new_Parameter("xRFRds",0.0);
    symbol_numeric xRFRls = *sys.new_Parameter("xRFRls",0.0);  
    
    symbol_numeric yRFRas = *sys.new_Parameter("yRFRas",0.0);
    symbol_numeric yRFRbs = *sys.new_Parameter("yRFRbs",0.0);
    symbol_numeric yRFRcs = *sys.new_Parameter("yRFRcs",0.0);
    symbol_numeric yRFRds = *sys.new_Parameter("yRFRds",0.0);
    symbol_numeric yRFRls = *sys.new_Parameter("yRFRls",0.0);     
    
    symbol_numeric zRFRas = *sys.new_Parameter("zRFRas",0.0);
    symbol_numeric zRFRbs = *sys.new_Parameter("zRFRbs",0.0);
    symbol_numeric zRFRcs = *sys.new_Parameter("zRFRcs",0.0);
    symbol_numeric zRFRds = *sys.new_Parameter("zRFRds",0.0);
    symbol_numeric zRFRls = *sys.new_Parameter("zRFRls",0.0);     
      
    ex slR_RFRx   = atomize_ex (sR_RFR - xRFRls);
    ex slR_RFRy   = atomize_ex (sR_RFR - yRFRls);
    ex slR_RFRz   = atomize_ex (sR_RFR - zRFRls);   

    ex spS_RFRx   = atomize_ex (xRFRds + slR_RFRx * ( xRFRcs + slR_RFRx * (xRFRbs + slR_RFRx * xRFRas ) ) );
    ex spS_RFRy   = atomize_ex (yRFRds + slR_RFRy * ( yRFRcs + slR_RFRy * (yRFRbs + slR_RFRy * yRFRas ) ) );
    ex spS_RFRz   = atomize_ex (zRFRds + slR_RFRz * ( zRFRcs + slR_RFRz * (zRFRbs + slR_RFRz * zRFRas ) ) );    

    ex dspS_RFRx = sys.diff ( spS_RFRx , sR_RFR);
    ex dspS_RFRy = sys.diff ( spS_RFRy , sR_RFR);
    
    ex dspS_RFR_mod = atomize_ex(sqrt(dspS_RFRx*dspS_RFRx + dspS_RFRy*dspS_RFRy));
    Vector3D pR_RFR  = *sys.new_Vector3D("pR_RFR", spS_RFRx + (L_RAIL/2.0) * dspS_RFRy / dspS_RFR_mod , spS_RFRy - (L_RAIL/2.0) *dspS_RFRx/dspS_RFR_mod ,0.0,"B_RAIL");
    
    symbol_numeric irrRFRz = *sys.new_Parameter("irrRFRz",0.0);
    Vector3D iR_RFR  = *sys.new_Vector3D("iR_RFR",0.0, 0.0 ,irrRFRz,"B_RAIL");
    
    Vector3D rR_RFR  = pR_RFR + fR_RFR + iR_RFR;  
     
    
    Vector3D t1R_RFR= sys.diff ( rR_RFR , sR_RFR);
    ...
    symbol_numeric sR_RFL = *sys.new_AuxCoordinate("sR_RFL","dsR_RFL","ddsR_RFL", -3.65595 ,0.0,0.0);
    ...
    ex slR_RFLp = atomize_ex (uR_RFL - lR_RFL);
    ex splineR_RFL =   atomize_ex (dR_RFL + slR_RFLp * ( cR_RFL + slR_RFLp * (bR_RFL + slR_RFLp * aR_RFL ) ));
    Vector3D fR_RFL = *sys.new_Vector3D("fR_RFL",0.0,uR_RFL,splineR_RFL,"B_RAIL");
    
    // spline rail
    symbol_numeric xRFLas = *sys.new_Parameter("xRFLas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric xRFLbs = *sys.new_Parameter("xRFLbs",0.0);
    symbol_numeric xRFLcs = *sys.new_Parameter("xRFLcs",1.0);
    symbol_numeric xRFLds = *sys.new_Parameter("xRFLds",0.0);
    symbol_numeric xRFLls = *sys.new_Parameter("xRFLls",0.0);  
    
    symbol_numeric yRFLas = *sys.new_Parameter("yRFLas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric yRFLbs = *sys.new_Parameter("yRFLbs",0.0);
    symbol_numeric yRFLcs = *sys.new_Parameter("yRFLcs",0.0);
    symbol_numeric yRFLds = *sys.new_Parameter("yRFLds",0.0);
    symbol_numeric yRFLls = *sys.new_Parameter("yRFLls",0.0);  
        
    symbol_numeric zRFLas = *sys.new_Parameter("zRFLas",0.0);
    symbol_numeric zRFLbs = *sys.new_Parameter("zRFLbs",0.0);
    symbol_numeric zRFLcs = *sys.new_Parameter("zRFLcs",0.0);
    symbol_numeric zRFLds = *sys.new_Parameter("zRFLds",0.0);
    symbol_numeric zRFLls = *sys.new_Parameter("zRFLls",0.0);      

    ex slR_RFLx   = atomize_ex (sR_RFL - xRFLls);
    ex slR_RFLy   = atomize_ex (sR_RFL - yRFLls);
    ex slR_RFLz   = atomize_ex (sR_RFL - zRFLls);    
    
    ex spS_RFLx   = atomize_ex (xRFLds + slR_RFLx * ( xRFLcs + slR_RFLx * (xRFLbs + slR_RFLx * xRFLas ) ) );
    ex spS_RFLy   = atomize_ex (yRFLds + slR_RFLy * ( yRFLcs + slR_RFLy * (yRFLbs + slR_RFLy * yRFLas ) ) );
    ex spS_RFLz   = atomize_ex (zRFLds + slR_RFLz * ( zRFLcs + slR_RFLz * (zRFLbs + slR_RFLz * zRFLas ) ) );   
    
    ex dspS_RFLx = sys.diff ( spS_RFLx , sR_RFL);
    ex dspS_RFLy = sys.diff ( spS_RFLy , sR_RFL);

    ex dspS_RFL_mod = atomize_ex(sqrt(dspS_RFLx*dspS_RFLx + dspS_RFLy*dspS_RFLy));
    Vector3D pR_RFL  = *sys.new_Vector3D("pR_RFL", spS_RFLx - (L_RAIL/2.0) * dspS_RFLy / dspS_RFL_mod , spS_RFLy + (L_RAIL/2.0) *dspS_RFLx/dspS_RFL_mod ,0.0,"B_RAIL");   


    symbol_numeric irrRFLz = *sys.new_Parameter("irrRFLz",0.0);
    Vector3D iR_RFL  = *sys.new_Vector3D("iR_RFL",0.0, 0.0 ,irrRFLz,"B_RAIL");
    
    Vector3D rR_RFL  = pR_RFL + fR_RFL + iR_RFL; 
     
    Vector3D t1R_RFL= sys.diff ( rR_RFL , sR_RFL);
...
cout << " RRR" <<endl;
    ...
    symbol_numeric sR_RRR = *sys.new_AuxCoordinate("sR_RRR","dsR_RRR","ddsR_RRR", -5.84095 ,0.0,0.0);
    ...
    ex slR_RRRp = atomize_ex (uR_RRR - lR_RRR);
    ex splineR_RRR =   atomize_ex (dR_RRR + slR_RRRp * ( cR_RRR + slR_RRRp * (bR_RRR + slR_RRRp * aR_RRR ) ));
    Vector3D fR_RRR = *sys.new_Vector3D("fR_RRR",0.0,uR_RRR,splineR_RRR,"B_RAIL");
    
    // spline rail
    symbol_numeric xRRRas = *sys.new_Parameter("xRRRas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric xRRRbs = *sys.new_Parameter("xRRRbs",0.0);
    symbol_numeric xRRRcs = *sys.new_Parameter("xRRRcs",1.0);
    symbol_numeric xRRRds = *sys.new_Parameter("xRRRds",0.0);
    symbol_numeric xRRRls = *sys.new_Parameter("xRRRls",0.0);  
    
    symbol_numeric yRRRas = *sys.new_Parameter("yRRRas",0.0);
    symbol_numeric yRRRbs = *sys.new_Parameter("yRRRbs",0.0);
    symbol_numeric yRRRcs = *sys.new_Parameter("yRRRcs",0.0);
    symbol_numeric yRRRds = *sys.new_Parameter("yRRRds",0.0);
    symbol_numeric yRRRls = *sys.new_Parameter("yRRRls",0.0);      
    
    symbol_numeric zRRRas = *sys.new_Parameter("zRRRas",0.0);
    symbol_numeric zRRRbs = *sys.new_Parameter("zRRRbs",0.0);
    symbol_numeric zRRRcs = *sys.new_Parameter("zRRRcs",0.0);
    symbol_numeric zRRRds = *sys.new_Parameter("zRRRds",0.0);
    symbol_numeric zRRRls = *sys.new_Parameter("zRRRls",0.0); 
          
    ex slR_RRRx   = atomize_ex (sR_RRR - xRRRls);
    ex slR_RRRy   = atomize_ex (sR_RRR - yRRRls);
    ex slR_RRRz   = atomize_ex (sR_RRR - zRRRls);
        
    ex spS_RRRx   = atomize_ex (xRRRds + slR_RRRx * ( xRRRcs + slR_RRRx * (xRRRbs + slR_RRRx * xRRRas ) ) );
    ex spS_RRRy   = atomize_ex (yRRRds + slR_RRRy * ( yRRRcs + slR_RRRy * (yRRRbs + slR_RRRy * yRRRas ) ) );
    ex spS_RRRz   = atomize_ex (zRRRds + slR_RRRz * ( zRRRcs + slR_RRRz * (zRRRbs + slR_RRRz * zRRRas ) ) );
        
    //ex spS_RRR   = 0;
    ex dspS_RRRx = sys.diff ( spS_RRRx , sR_RRR);
    ex dspS_RRRy = sys.diff ( spS_RRRy , sR_RRR);
    ex dspS_RRR_mod = atomize_ex(sqrt(dspS_RRRx*dspS_RRRx + dspS_RRRy*dspS_RRRy));
    Vector3D pR_RRR  = *sys.new_Vector3D("pR_RRR", spS_RRRx + (L_RAIL/2.0) * dspS_RRRy / dspS_RRR_mod , spS_RRRy - (L_RAIL/2.0) *dspS_RRRx/dspS_RFR_mod ,0.0,"B_RAIL");   
    
    symbol_numeric irrRRRz = *sys.new_Parameter("irrRRRz",0.0);
    Vector3D iR_RRR  = *sys.new_Vector3D("iR_RRR",0.0, 0.0 ,irrRRRz,"B_RAIL");
    
    Vector3D rR_RRR  = pR_RRR + fR_RRR + iR_RRR;    

    Vector3D t1R_RRR= sys.diff ( rR_RRR , sR_RRR);
...
cout << " RRL" <<endl;
    ...
    symbol_numeric sR_RRL = *sys.new_AuxCoordinate("sR_RRL","dsR_RRL","ddsR_RRL", -5.84095 ,0.0,0.0);
    ...
    ex slR_RRLp = atomize_ex (uR_RRL - lR_RRL);
    ex splineR_RRL =   atomize_ex (dR_RRL + slR_RRLp * ( cR_RRL + slR_RRLp * (bR_RRL + slR_RRLp * aR_RRL ) ));
    Vector3D fR_RRL = *sys.new_Vector3D("fR_RRL",0.0,uR_RRL,splineR_RRL,"B_RAIL");
    
    // spline rail
    symbol_numeric xRRLas = *sys.new_Parameter("xRRLas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric xRRLbs = *sys.new_Parameter("xRRLbs",0.0);
    symbol_numeric xRRLcs = *sys.new_Parameter("xRRLcs",1.0);
    symbol_numeric xRRLds = *sys.new_Parameter("xRRLds",0.0);
    symbol_numeric xRRLls = *sys.new_Parameter("xRRLls",0.0);  
    
    symbol_numeric yRRLas = *sys.new_Parameter("yRRLas",0.0);
    symbol_numeric yRRLbs = *sys.new_Parameter("yRRLbs",0.0);
    symbol_numeric yRRLcs = *sys.new_Parameter("yRRLcs",0.0);
    symbol_numeric yRRLds = *sys.new_Parameter("yRRLds",0.0);
    symbol_numeric yRRLls = *sys.new_Parameter("yRRLls",0.0);      
      
    symbol_numeric zRRLas = *sys.new_Parameter("zRRLas",0.0);
    symbol_numeric zRRLbs = *sys.new_Parameter("zRRLbs",0.0);
    symbol_numeric zRRLcs = *sys.new_Parameter("zRRLcs",0.0);
    symbol_numeric zRRLds = *sys.new_Parameter("zRRLds",0.0);
    symbol_numeric zRRLls = *sys.new_Parameter("zRRLls",0.0);       
      
    ex slR_RRLx   = atomize_ex (sR_RRL - xRRLls);
    ex slR_RRLy   = atomize_ex (sR_RRL - yRRLls);
    ex slR_RRLz   = atomize_ex (sR_RRL - zRRLls);
        
    ex spS_RRLx   = atomize_ex (xRRLds + slR_RRLx * ( xRRLcs + slR_RRLx * (xRRLbs + slR_RRLx * xRRLas ) ) );
    ex spS_RRLy   = atomize_ex (yRRLds + slR_RRLy * ( yRRLcs + slR_RRLy * (yRRLbs + slR_RRLy * yRRLas ) ) );
    ex spS_RRLz   = atomize_ex (zRRLds + slR_RRLz * ( zRRLcs + slR_RRLz * (zRRLbs + slR_RRLz * zRRLas ) ) );
        
    //ex spS_RRL   = 0;
    ex dspS_RRLx = sys.diff ( spS_RRLx , sR_RRL);
    ex dspS_RRLy = sys.diff ( spS_RRLy , sR_RRL);
    ex dspS_RRL_mod = atomize_ex(sqrt(dspS_RRLx*dspS_RRLx + dspS_RRLy*dspS_RRLy));
    Vector3D pR_RRL  = *sys.new_Vector3D("pR_RRL", spS_RRLx - (L_RAIL/2.0) * dspS_RRLy / dspS_RRL_mod , spS_RRLy + (L_RAIL/2.0) *dspS_RRLx/dspS_RRL_mod ,0.0,"B_RAIL");    

    symbol_numeric irrRRLz = *sys.new_Parameter("irrRRLz",0.0); 
    Vector3D iR_RRL  = *sys.new_Vector3D("iR_RRL",0.0, 0.0 ,irrRRLz,"B_RAIL");
    
    Vector3D rR_RRL  = pR_RRL + fR_RRL + iR_RRL;
    

    Vector3D t1R_RRL= sys.diff ( rR_RRL , sR_RRL);
...
cout << " FFR" <<endl;   
    ...
    symbol_numeric sR_FFR = *sys.new_AuxCoordinate("sR_FFR","dsR_FFR","ddsR_FFR", 5.84404 ,0.0,0.0);
    ...
    ex slR_FFRp = atomize_ex (uR_FFR - lR_FFR);
    ex splineR_FFR =   atomize_ex (dR_FFR + slR_FFRp * ( cR_FFR + slR_FFRp * (bR_FFR + slR_FFRp * aR_FFR ) ));
    Vector3D fR_FFR = *sys.new_Vector3D("fR_FFR",0.0,uR_FFR,splineR_FFR,"B_RAIL");
    
    // spline rail
    symbol_numeric xFFRas = *sys.new_Parameter("xFFRas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric xFFRbs = *sys.new_Parameter("xFFRbs",0.0);
    symbol_numeric xFFRcs = *sys.new_Parameter("xFFRcs",1.0);
    symbol_numeric xFFRds = *sys.new_Parameter("xFFRds",0.0);
    symbol_numeric xFFRls = *sys.new_Parameter("xFFRls",0.0);  
    
    symbol_numeric yFFRas = *sys.new_Parameter("yFFRas",0.0);
    symbol_numeric yFFRbs = *sys.new_Parameter("yFFRbs",0.0);
    symbol_numeric yFFRcs = *sys.new_Parameter("yFFRcs",0.0);
    symbol_numeric yFFRds = *sys.new_Parameter("yFFRds",0.0);
    symbol_numeric yFFRls = *sys.new_Parameter("yFFRls",0.0);  
    
    symbol_numeric zFFRas = *sys.new_Parameter("zFFRas",0.0);
    symbol_numeric zFFRbs = *sys.new_Parameter("zFFRbs",0.0);
    symbol_numeric zFFRcs = *sys.new_Parameter("zFFRcs",0.0);
    symbol_numeric zFFRds = *sys.new_Parameter("zFFRds",0.0);
    symbol_numeric zFFRls = *sys.new_Parameter("zFFRls",0.0);          
      
    ex slR_FFRx   = atomize_ex (sR_FFR - xFFRls);
    ex slR_FFRy   = atomize_ex (sR_FFR - yFFRls);
    ex slR_FFRz   = atomize_ex (sR_FFR - zFFRls);    
    
    ex spS_FFRx   = atomize_ex (xFFRds + slR_FFRx * ( xFFRcs + slR_FFRx * (xFFRbs + slR_FFRx * xFFRas ) ) );
    ex spS_FFRy   = atomize_ex (yFFRds + slR_FFRy * ( yFFRcs + slR_FFRy * (yFFRbs + slR_FFRy * yFFRas ) ) );
    ex spS_FFRz   = atomize_ex (zFFRds + slR_FFRz * ( zFFRcs + slR_FFRz * (zFFRbs + slR_FFRz * zFFRas ) ) );   

    ex dspS_FFRx = sys.diff ( spS_FFRx , sR_FFR);
    ex dspS_FFRy = sys.diff ( spS_FFRy , sR_FFR);
    ex dspS_FFR_mod = atomize_ex(sqrt(dspS_FFRx*dspS_FFRx + dspS_FFRy*dspS_FFRy));
    Vector3D pR_FFR  = *sys.new_Vector3D("pR_FFR", spS_FFRx + (L_RAIL/2.0) * dspS_FFRy / dspS_FFR_mod , spS_FFRy - (L_RAIL/2.0) *dspS_FFRx/dspS_FFR_mod ,0.0,"B_RAIL");   

    symbol_numeric irrFFRz = *sys.new_Parameter("irrFFRz",0.0);
    Vector3D iR_FFR  = *sys.new_Vector3D("iR_FFR",0.0, 0.0 ,irrFFRz,"B_RAIL");
    
    Vector3D rR_FFR  = pR_FFR + fR_FFR + iR_FFR;

    Vector3D t1R_FFR= sys.diff ( rR_FFR , sR_FFR);
...
cout << " FFL" <<endl;
    ...
    symbol_numeric sR_FFL = *sys.new_AuxCoordinate("sR_FFL","dsR_FFL","ddsR_FFL", 5.84404 ,0.0,0.0);
    ...
    ex slR_FFLp = atomize_ex (uR_FFL - lR_FFL);
    ex splineR_FFL =   atomize_ex (dR_FFL + slR_FFLp * ( cR_FFL + slR_FFLp * (bR_FFL + slR_FFLp * aR_FFL ) ));
    Vector3D fR_FFL = *sys.new_Vector3D("fR_FFL",0.0,uR_FFL,splineR_FFL,"B_RAIL");
    
    // spline rail
    symbol_numeric xFFLas = *sys.new_Parameter("xFFLas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric xFFLbs = *sys.new_Parameter("xFFLbs",0.0);
    symbol_numeric xFFLcs = *sys.new_Parameter("xFFLcs",1.0);
    symbol_numeric xFFLds = *sys.new_Parameter("xFFLds",0.0);
    symbol_numeric xFFLls = *sys.new_Parameter("xFFLls",0.0);  
    
    symbol_numeric yFFLas = *sys.new_Parameter("yFFLas",0.0);
    symbol_numeric yFFLbs = *sys.new_Parameter("yFFLbs",0.0);
    symbol_numeric yFFLcs = *sys.new_Parameter("yFFLcs",0.0);
    symbol_numeric yFFLds = *sys.new_Parameter("yFFLds",0.0);
    symbol_numeric yFFLls = *sys.new_Parameter("yFFLls",0.0);
    
    symbol_numeric zFFLas = *sys.new_Parameter("zFFLas",0.0);
    symbol_numeric zFFLbs = *sys.new_Parameter("zFFLbs",0.0);
    symbol_numeric zFFLcs = *sys.new_Parameter("zFFLcs",0.0);
    symbol_numeric zFFLds = *sys.new_Parameter("zFFLds",0.0);
    symbol_numeric zFFLls = *sys.new_Parameter("zFFLls",0.0);          
      
    ex slR_FFLx   = atomize_ex (sR_FFL - xFFLls);
    ex slR_FFLy   = atomize_ex (sR_FFL - yFFLls);
    ex slR_FFLz   = atomize_ex (sR_FFL - zFFLls);
        
    ex spS_FFLx   = atomize_ex (xFFLds + slR_FFLx * ( xFFLcs + slR_FFLx * (xFFLbs + slR_FFLx * xFFLas ) ) );
    ex spS_FFLy   = atomize_ex (yFFLds + slR_FFLy * ( yFFLcs + slR_FFLy * (yFFLbs + slR_FFLy * yFFLas ) ) );
    ex spS_FFLz   = atomize_ex (zFFLds + slR_FFLz * ( zFFLcs + slR_FFLz * (zFFLbs + slR_FFLz * zFFLas ) ) );
    
    ex dspS_FFLx = sys.diff ( spS_FFLx , sR_FFL);
    ex dspS_FFLy = sys.diff ( spS_FFLy , sR_FFL);
    ex dspS_FFL_mod = atomize_ex(sqrt(dspS_FFLx*dspS_FFLx + dspS_FFLy*dspS_FFLy));
    Vector3D pR_FFL  = *sys.new_Vector3D("pR_FFL", spS_FFLx - (L_RAIL/2.0) * dspS_FFLy / dspS_FFL_mod , spS_FFLy + (L_RAIL/2.0) *dspS_FFLx/dspS_FFL_mod ,0.0,"B_RAIL");
    
    symbol_numeric irrFFLz = *sys.new_Parameter("irrFFLz",0.0);
    Vector3D iR_FFL  = *sys.new_Vector3D("iR_FFL",0.0, 0.0 ,irrFFLz,"B_RAIL");
    
    Vector3D rR_FFL  = pR_FFL + fR_FFL + iR_FFL;
   
    Vector3D t1R_FFL= sys.diff ( rR_FFL , sR_FFL);
...
cout << " FRR " <<endl;
    ...
    symbol_numeric sR_FRR = *sys.new_AuxCoordinate("sR_FRR","dsR_FRR","ddsR_FRR", 3.65904,0.0,0.0);
    ...
    ex slR_FRRp = atomize_ex (uR_FRR - lR_FRR);
    ex splineR_FRR =   atomize_ex (dR_FRR + slR_FRRp * ( cR_FRR + slR_FRRp * (bR_FRR + slR_FRRp * aR_FRR ) ));
    Vector3D fR_FRR = *sys.new_Vector3D("fR_FRR",0.0,uR_FRR,splineR_FRR,"B_RAIL");
    
    // spline rail
    symbol_numeric xFRRas = *sys.new_Parameter("xFRRas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric xFRRbs = *sys.new_Parameter("xFRRbs",0.0);
    symbol_numeric xFRRcs = *sys.new_Parameter("xFRRcs",1.0);
    symbol_numeric xFRRds = *sys.new_Parameter("xFRRds",0.0);
    symbol_numeric xFRRls = *sys.new_Parameter("xFRRls",0.0);  
    
    symbol_numeric yFRRas = *sys.new_Parameter("yFRRas",0.0);
    symbol_numeric yFRRbs = *sys.new_Parameter("yFRRbs",0.0);
    symbol_numeric yFRRcs = *sys.new_Parameter("yFRRcs",0.0);
    symbol_numeric yFRRds = *sys.new_Parameter("yFRRds",0.0);
    symbol_numeric yFRRls = *sys.new_Parameter("yFRRls",0.0);     
    
    symbol_numeric zFRRas = *sys.new_Parameter("zFRRas",0.0);
    symbol_numeric zFRRbs = *sys.new_Parameter("zFRRbs",0.0);
    symbol_numeric zFRRcs = *sys.new_Parameter("zFRRcs",0.0);
    symbol_numeric zFRRds = *sys.new_Parameter("zFRRds",0.0);
    symbol_numeric zFRRls = *sys.new_Parameter("zFRRls",0.0);      
      
    ex slR_FRRx   = atomize_ex (sR_FRR - xFRRls);
    ex slR_FRRy   = atomize_ex (sR_FRR - yFRRls);
    ex slR_FRRz   = atomize_ex (sR_FRR - zFRRls);    
    
    ex spS_FRRx   = atomize_ex (xFRRds + slR_FRRx * ( xFRRcs + slR_FRRx * (xFRRbs + slR_FRRx * xFRRas ) ) );
    ex spS_FRRy   = atomize_ex (yFRRds + slR_FRRy * ( yFRRcs + slR_FRRy * (yFRRbs + slR_FRRy * yFRRas ) ) );
    ex spS_FRRz   = atomize_ex (zFRRds + slR_FRRz * ( zFRRcs + slR_FRRz * (zFRRbs + slR_FRRz * zFRRas ) ) );    
    

    ex dspS_FRRx = sys.diff ( spS_FRRx , sR_FRR);
    ex dspS_FRRy = sys.diff ( spS_FRRy , sR_FRR);
    ex dspS_FRR_mod = atomize_ex(sqrt(dspS_FRRx*dspS_FRRx + dspS_FRRy*dspS_FRRy));
    Vector3D pR_FRR  = *sys.new_Vector3D("pR_FRR", spS_FRRx + (L_RAIL/2.0) * dspS_FRRy / dspS_FRR_mod , spS_FRRy - (L_RAIL/2.0) *dspS_FRRx/dspS_FRR_mod ,0.0,"B_RAIL");
    
    symbol_numeric irrFRRz = *sys.new_Parameter("irrFRRz",0.0);
    Vector3D iR_FRR  = *sys.new_Vector3D("iR_FRR",0.0, 0.0 ,irrFRRz,"B_RAIL");
    
    Vector3D rR_FRR  = pR_FRR + fR_FRR + iR_FRR;  

    Vector3D t1R_FRR= sys.diff ( rR_FRR , sR_FRR);
    ...
    symbol_numeric sR_FRL = *sys.new_AuxCoordinate("sR_FRL","dsR_FRL","ddsR_FRL", 3.65904 ,0.0,0.0);
    ...
    ex slR_FRLp = atomize_ex (uR_FRL - lR_FRL);
    ex splineR_FRL =   atomize_ex (dR_FRL + slR_FRLp * ( cR_FRL + slR_FRLp * (bR_FRL + slR_FRLp * aR_FRL ) ));
    Vector3D fR_FRL = *sys.new_Vector3D("fR_FRL",0.0,uR_FRL,splineR_FRL,"B_RAIL");
    
    // spline rail
    symbol_numeric xFRLas = *sys.new_Parameter("xFRLas",0.0);//spline's coefficients left rail ppsition
    symbol_numeric xFRLbs = *sys.new_Parameter("xFRLbs",0.0);
    symbol_numeric xFRLcs = *sys.new_Parameter("xFRLcs",1.0);
    symbol_numeric xFRLds = *sys.new_Parameter("xFRLds",0.0);
    symbol_numeric xFRLls = *sys.new_Parameter("xFRLls",0.0);  
    
    symbol_numeric yFRLas = *sys.new_Parameter("yFRLas",0.0);
    symbol_numeric yFRLbs = *sys.new_Parameter("yFRLbs",0.0);
    symbol_numeric yFRLcs = *sys.new_Parameter("yFRLcs",0.0);
    symbol_numeric yFRLds = *sys.new_Parameter("yFRLds",0.0);
    symbol_numeric yFRLls = *sys.new_Parameter("yFRLls",0.0);      
    
    symbol_numeric zFRLas = *sys.new_Parameter("zFRLas",0.0);
    symbol_numeric zFRLbs = *sys.new_Parameter("zFRLbs",0.0);
    symbol_numeric zFRLcs = *sys.new_Parameter("zFRLcs",0.0);
    symbol_numeric zFRLds = *sys.new_Parameter("zFRLds",0.0);
    symbol_numeric zFRLls = *sys.new_Parameter("zFRLls",0.0);       
      
    ex slR_FRLx   = atomize_ex (sR_FRL - xFRLls);
    ex slR_FRLy   = atomize_ex (sR_FRL - yFRLls);
    ex slR_FRLz   = atomize_ex (sR_FRL - zFRLls);
        
    ex spS_FRLx   = atomize_ex (xFRLds + slR_FRLx * ( xFRLcs + slR_FRLx * (xFRLbs + slR_FRLx * xFRLas ) ) );
    ex spS_FRLy   = atomize_ex (yFRLds + slR_FRLy * ( yFRLcs + slR_FRLy * (yFRLbs + slR_FRLy * yFRLas ) ) );
    ex spS_FRLz   = atomize_ex (zFRLds + slR_FRLz * ( zFRLcs + slR_FRLz * (zFRLbs + slR_FRLz * zFRLas ) ) );
    
    ex dspS_FRLx = sys.diff ( spS_FRLx , sR_FRL);
    ex dspS_FRLy = sys.diff ( spS_FRLy , sR_FRL);
    ex dspS_FRL_mod = atomize_ex(sqrt(dspS_FRLx*dspS_FRLx + dspS_FRLy*dspS_FRLy));
    Vector3D pR_FRL  = *sys.new_Vector3D("pR_FRL", spS_FRLx - (L_RAIL/2.0) * dspS_FRLy / dspS_FRL_mod , spS_FRLy + (L_RAIL/2.0) *dspS_FRLx/dspS_FRL_mod ,0.0,"B_RAIL");
      
    symbol_numeric irrFRLz = *sys.new_Parameter("irrFRLz",0.0);
    Vector3D iR_FRL  = *sys.new_Vector3D("iR_FRL",0.0, 0.0 ,irrFRLz,"B_RAIL");

    Vector3D rR_FRL  = pR_FRL + fR_FRL + iR_FRL;

    Vector3D t1R_FRL= sys.diff ( rR_FRL , sR_FRL);
    ...
    // ************************************************************************************************
//~ cout << unatomize(rR_RRR)<< endl;
//~ cout << unatomize(rR_RRL)<< endl;
//~ cout << unatomize(rR_RFR)<< endl;
//~ cout << unatomize(rR_RFL)<< endl;
//~ cout << unatomize(rR_FRR)<< endl;
//~ cout << unatomize(rR_FRL)<< endl;
//~ cout << unatomize(rR_FFR)<< endl;
//~ cout << unatomize(rR_FFL)<< endl;
// ************************************************************************************************
...
// ************************************************************************************************
    ...
    symbol_numeric epsiRFR = *sys.new_Parameter("epsiRFR",1.0);
    symbol_numeric epsiRFL = *sys.new_Parameter("epsiRFL",1.0);
    symbol_numeric epsiRRR = *sys.new_Parameter("epsiRRR",1.0);
    symbol_numeric epsiRRL = *sys.new_Parameter("epsiRRL",1.0);
    symbol_numeric epsiFFR = *sys.new_Parameter("epsiFFR",1.0);
    symbol_numeric epsiFFL = *sys.new_Parameter("epsiFFL",1.0);
    symbol_numeric epsiFRR = *sys.new_Parameter("epsiFRR",1.0);
    symbol_numeric epsiFRL = *sys.new_Parameter("epsiFRL",1.0);    
    ...
    Matrix E_RFR(3,3); 
    E_RFR(0,0) = epsiRFR; E_RFR(1,1) = epsiRFR; E_RFR(2,2) = 0;
    //E_RFR(0,0) = 1;       E_RFR(1,1) = 1;       E_RFR(2,2) = 1;
    ...
    Matrix CKRFR = dQ_dKFRFR * E_RFR * D_RFR * sys.jacobian(SlipRFR.transpose(),dq);
    ...
    Matrix E_RFL(3,3); 
    E_RFL(0,0) = epsiRFL; E_RFL(1,1) = epsiRFL; E_RFL(2,2) = 0;
    //E_RFL(0,0) = 1;       E_RFL(1,1) = 1;       E_RFL(2,2) = 1;
    ...
    Matrix CKRFL = dQ_dKFRFL * E_RFL * D_RFL*sys.jacobian(SlipRFL.transpose(),dq);
    ...
    Matrix E_RRR(3,3); 
    E_RRR(0,0) = epsiRRR; E_RRR(1,1) = epsiRRR; E_RRR(2,2) = 0;
    //E_RRR(0,0) = 1;       E_RRR(1,1) = 1;       E_RRR(2,2) = 1;   
    ...
    Matrix CKRRR = dQ_dKFRRR * E_RRR * D_RRR*sys.jacobian(SlipRRR.transpose(),dq);
    ...
    Matrix E_RRL(3,3); 
    E_RRL(0,0) = epsiRRL; E_RRL(1,1) = epsiRRL; E_RRL(2,2) = 0;
    //E_RRL(0,0) = 1;       E_RRL(1,1) = 1;       E_RRL(2,2) = 1;      
    ...
    Matrix CKRRL = dQ_dKFRRL * E_RRL * D_RRL*sys.jacobian(SlipRRL.transpose(),dq);
    ...
    Matrix E_FFR(3,3); 
    E_FFR(0,0) = epsiFFR; E_FFR(1,1) = epsiFFR; E_FFR(2,2) = 0;
    //E_FFR(0,0) = 1;       E_FFR(1,1) = 1;       E_FFR(2,2) = 1; 
    ...
    Matrix CKFFR = dQ_dKFFFR * E_FFR * D_FFR*sys.jacobian(SlipFFR.transpose(),dq);
    ...
    Matrix E_FFL(3,3); 
    E_FFL(0,0) = epsiFFL; E_FFL(1,1) = epsiFFL; E_FFL(2,2) = 0;
    //E_FFL(0,0) = 1;       E_FFL(1,1) = 1;       E_FFL(2,2) = 1; 
    ...
    Matrix CKFFL = dQ_dKFFFL * E_FFL * D_FFL*sys.jacobian(SlipFFL.transpose(),dq);
    ...
    Matrix E_FRR(3,3); 
    E_FRR(0,0) = epsiFRR; E_FRR(1,1) = epsiFRR; E_FRR(2,2) = 0;
    //E_FRR(0,0) = 1;       E_FRR(1,1) = 1;       E_FRR(2,2) = 1; 
    ...
    Matrix CKFRR = dQ_dKFFRR * E_FRR *  D_FRR * sys.jacobian(SlipFRR.transpose(),dq);
    ...
    Matrix E_FRL(3,3); 
    E_FRL(0,0) = epsiFRL; E_FRL(1,1) = epsiFRL; E_FRL(2,2) = 0;
    //E_FRL(0,0) = 1;       E_FRL(1,1) = 1;       E_FRL(2,2) = 1; 
    ...
    Matrix CKFRL = dQ_dKFFRL * E_FRL * D_FRL*sys.jacobian(SlipFRL.transpose(),dq);
...
// sustituir las fuerzas de kalker (inputs) por cero en Q y exportar el nuevo Q (Q2)
    ...
    Matrix M   = *sys.get_Matrix( "M" );
    Matrix MDelta = Matrix("MDelta",1,2,&M,&Delta);
    lst new_atom_list_MDelta, new_exp_list_MDelta;
    matrix_list_optimize (MDelta, new_atom_list_MDelta,new_exp_list_MDelta);
    sys.export_Matrix_C	("MDelta","_MDelta",MDelta, new_atom_list_MDelta, new_exp_list_MDelta ,ORDER);
    sys.export_function_MATLAB("MDelta", "MDelta_",MDelta, new_atom_list_MDelta, new_exp_list_MDelta,"q,qaux,dq,time,param,inputs");     
// ************************************************************************************************
// 	Energy Equations
// ************************************************************************************************
