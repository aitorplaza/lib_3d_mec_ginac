...
//Kinematical parameter definition
    ...
    symbol_numeric L2x = *sys.new_Parameter("L2x",2.726387);
//  symbol_numeric L2y = *sys.new_Parameter("L2y",3.0);
    symbol_numeric L2z = *sys.new_Parameter("L2z",1.97181);//2.0 before
    ...
    symbol_numeric LS2x1 = *sys.new_Parameter("LS2x1",-0.55673698);// antes 1.1697
    symbol_numeric LS2y1 = *sys.new_Parameter("LS2y1",0.8777);
    symbol_numeric LS2z1 = *sys.new_Parameter("LS2z1",1.09411);	//antes 1.1308

    // The second group of silent-blocks (the four silent-blocks of the generator)
    #ifdef ALIGNED
        symbol_numeric LS2x2 = *sys.new_Parameter("LS2x2",3.3688463);
        symbol_numeric LS2x3 = *sys.new_Parameter("LS2x3",4.4361796);
        symbol_numeric LS2y2 = *sys.new_Parameter("LS2y2",0.8);
        symbol_numeric LS2y3 = *sys.new_Parameter("LS2y3",0.8);
        symbol_numeric LS2z2 = *sys.new_Parameter("LS2z2",2.1657495);// 2.2808
    #else
        symbol_numeric LS2x2 = *sys.new_Parameter("LS2x2",3.36884633333333);
        symbol_numeric LS2x3 = *sys.new_Parameter("LS2x3",4.43617966666667);
        symbol_numeric LS2y2 = *sys.new_Parameter("LS2y2",0.8);
        symbol_numeric LS2y3 = *sys.new_Parameter("LS2y3",0.8);
        symbol_numeric LS2z2 = *sys.new_Parameter("LS2z2",2.1657495);
    #endif

    // Distance between Origin of the nacelle (O2, at the center of yaw Bearing)
    // and the Brake point (in the HHS)
    #ifdef ALIGNED
        symbol_numeric LB2x = *sys.new_Parameter("LB2x",0.614113);//L3x2+L4x-L2x+LB5x
        symbol_numeric LB2y = *sys.new_Parameter("LB2y",0.3);
        symbol_numeric LB2z = *sys.new_Parameter("LB2z",1.97181);
    #else
        symbol_numeric LB2x = *sys.new_Parameter("LB2x",0.614113);//L3x2+L4x-L2x+LB5x
        symbol_numeric LB2y = *sys.new_Parameter("LB2y",0.3);
        symbol_numeric LB2z = *sys.new_Parameter("LB2z",1.97181);
    ...
    symbol_numeric LS4x = *sys.new_Parameter("LS4x",0.49975);   // L4Sx is common for the two silent-blocks
    symbol_numeric LS4y = *sys.new_Parameter("LS4y",1.0);   // 0.8777 LS4y is the half distance between silent-blocks
    symbol_numeric LS4z = *sys.new_Parameter("LS4z",0.8777);   // LS4z is common for the two silent-blocks
    ...
    symbol_numeric L6x = *sys.new_Parameter("L6x",1.2284);

    // Distance from the coupling point of solid 5 (O5) to the braking point (FF)
    symbol_numeric LB5x = *sys.new_Parameter("LB5x",0.1712);//0.25
    symbol_numeric LB5y = *sys.new_Parameter("LB5y",0.3);//0.5
    ...
    symbol_numeric LS7x1 = *sys.new_Parameter("LS7x1",0.6302+(2.0/3.0)*3.202);//draw 0.8
    symbol_numeric LS7x2 = *sys.new_Parameter("LS7x2",0.6302+(2.0/3.0)*3.202);//draw 2.45
    symbol_numeric LS7y1 = *sys.new_Parameter("LS7y1",0.80000);
    symbol_numeric LS7y2 = *sys.new_Parameter("LS7y2",0.80000);
    #ifdef ALIGNED
        symbol_numeric LS7z = *sys.new_Parameter("LS7z",0.8000);
    #else
        symbol_numeric LS7z = *sys.new_Parameter("LS7z",0.8000);
    #endif
    ...
    symbol_numeric LGx1 = *sys.new_Parameter("LGx1",0.4318);
    symbol_numeric LGx2 = *sys.new_Parameter("LGx2",0.7208);   
    symbol_numeric LGx3 = *sys.new_Parameter("LGx3",0.8670);
    symbol_numeric LGx4 = *sys.new_Parameter("LGx4",1.4892);
    symbol_numeric LGx5 = *sys.new_Parameter("LGx5",0.8534);
    symbol_numeric LGx6 = *sys.new_Parameter("LGx6",1.4994);
    symbol_numeric LGx7 = *sys.new_Parameter("LGx7",0.8534);  
    symbol_numeric LGx8 = *sys.new_Parameter("LGx8",1.4994);//same as L4x    
    
    symbol_numeric LGr1 = *sys.new_Parameter("LGr1",0.4480); 
    
    symbol_numeric LGy1 = *sys.new_Parameter("LGy1",0.5232);   
    symbol_numeric LGz1 = *sys.new_Parameter("LGz1",0.8001);   
      
    symbol_numeric LGy2 = *sys.new_Parameter("LGy2",0.0);//same as L4y
    symbol_numeric LGz2 = *sys.new_Parameter("LGz2",0.9939395); //same as L4z


    //Teeth number
    symbol_numeric Z_Ring = *sys.new_Parameter("Z_Ring",190);
    symbol_numeric Z_Sun = *sys.new_Parameter("Z_Sun",34);
    symbol_numeric Z_Pla = *sys.new_Parameter("Z_Pla",78);
    symbol_numeric Z_SecSt1 = *sys.new_Parameter("Z_SecSt1",192);//Atached to Sun solid
    symbol_numeric Z_SecSt2 = *sys.new_Parameter("Z_SecSt2",47);//Atached to SecSt solid
    symbol_numeric Z_ThiSt1 = *sys.new_Parameter("Z_ThiSt1",194);//Atached to SecSt solid
    symbol_numeric Z_ThiSt2 = *sys.new_Parameter("Z_ThiSt2",54);//Atached to 5 solid
...
//    Solid * SolRTower=sys.new_Solid("RTower","O","xyz","./solids/SolRTower.stl",0.999,0.999,0.999,1.0); 
    Solid * SolR2=sys.new_Solid("R2","O2","B2a","./solids/SolR2.osgt",0.4,0.4,0.5,1.0); //Changed
    Solid * SolR3=sys.new_Solid("R3","O3","B3","./solids/SolR3.stl",0.98,0.98,0.98,1.0);
    Solid * SolR4=sys.new_Solid("R4","O4","B4","./solids/SolR4.stl",1.0,1.0,0.0,0.5);
    Solid * SolR5=sys.new_Solid("R5","O5","B5","./solids/SolR5.stl",1.0,0.0,1.0,1.0);
    Solid * SolR6=sys.new_Solid("R6","O6","B6","./solids/SolR6.stl",0.0,1.0,0.0,1.0);
    Solid * SolR7=sys.new_Solid("R7","O7","B7","./solids/SolR7.stl",0.0,0.0,1.0,1.0);
    Solid * SolRSun=sys.new_Solid("RSun","OSun","BSun","./solids/SolRSun.ive",1.0,0.4,0.4,1.0);
    Solid * SolRPla1=sys.new_Solid("RPla1","OPla1","BPla1","./solids/SolRPla1.stl",0.4,0.8,0.8,1.0);
    Solid * SolRPla2=sys.new_Solid("RPla2","OPla2","BPla2","./solids/SolRPla2.stl",0.4,0.8,0.8,1.0);
    Solid * SolRPla3=sys.new_Solid("RPla3","OPla3","BPla3","./solids/SolRPla3.stl",0.4,0.8,0.8,1.0);
    Solid * SolRSecSt=sys.new_Solid("RSecSt","OSecSt","BSecSt","./solids/SolRSecSt.stl",0.4,0.6,0.2,1.0);
...
//  symbol_numeric  Izz1 = *sys.new_Parameter("Izz1",1.0);  symbol_numeric  Iyz1 = *sys.new_Parameter("Iyz1",0.0);
    ...
    symbol_numeric  m2 = *sys.new_Parameter("m2",11000.0);  symbol_numeric  mx2 = *sys.new_Parameter("mx2",0.0);
    symbol_numeric  my2 = *sys.new_Parameter("my2",0.0);    symbol_numeric  mz2 = *sys.new_Parameter("mz2",0.0);
    symbol_numeric  Ixx2 = *sys.new_Parameter("Ixx2",55687.5);  symbol_numeric  Ixy2 = *sys.new_Parameter("Ixy2",0.0);
    symbol_numeric  Iyy2 = *sys.new_Parameter("Iyy2",55687.5);  symbol_numeric  Ixz2 = *sys.new_Parameter("Ixz2",0.0);
    symbol_numeric  Izz2 = *sys.new_Parameter("Izz2",55687.5);  symbol_numeric  Iyz2 = *sys.new_Parameter("Iyz2",0.0);

    // Dynamic Parameters of Bar 3
    symbol_numeric  m3 = *sys.new_Parameter("m3",41097);  symbol_numeric  mx3 = *sys.new_Parameter("mx3",-21.5686);
    symbol_numeric  my3 = *sys.new_Parameter("my3",0.0);    symbol_numeric  mz3 = *sys.new_Parameter("mz3",0.0);
    symbol_numeric  Ixx3 = *sys.new_Parameter("Ixx3",208870000);  symbol_numeric  Ixy3 = *sys.new_Parameter("Ixy3",0.0);
    symbol_numeric  Iyy3 = *sys.new_Parameter("Iyy3",104570000);  symbol_numeric  Ixz3 = *sys.new_Parameter("Ixz3",0.0);
    symbol_numeric  Izz3 = *sys.new_Parameter("Izz3",104570000);  symbol_numeric  Iyz3 = *sys.new_Parameter("Iyz3",0.0);

    // Dynamic Parameters of Bar 4
    symbol_numeric  m4 = *sys.new_Parameter("m4",7803.2);  symbol_numeric  mx4 = *sys.new_Parameter("mx4",6634.1);
    symbol_numeric  my4 = *sys.new_Parameter("my4",0.0);    symbol_numeric  mz4 = *sys.new_Parameter("mz4",0.0);
    symbol_numeric  Ixx4 = *sys.new_Parameter("Ixx4",5259);  symbol_numeric  Ixy4 = *sys.new_Parameter("Ixy4",0.0);
    symbol_numeric  Iyy4 = *sys.new_Parameter("Iyy4",10022);  symbol_numeric  Ixz4 = *sys.new_Parameter("Ixz4",0.0);
    symbol_numeric  Izz4 = *sys.new_Parameter("Izz4",10022);  symbol_numeric  Iyz4 = *sys.new_Parameter("Iyz4",0.0);

    // Dynamic Parameters of Bar 5
    symbol_numeric  m5 = *sys.new_Parameter("m5",498.1659);  symbol_numeric  mx5 = *sys.new_Parameter("mx5",197.0907);
    symbol_numeric  my5 = *sys.new_Parameter("my5",0.0);    symbol_numeric  mz5 = *sys.new_Parameter("mz5",0.0);
    symbol_numeric  Ixx5 = *sys.new_Parameter("Ixx5",2.2316);  symbol_numeric  Ixy5 = *sys.new_Parameter("Ixy5",0.0);
    symbol_numeric  Iyy5 = *sys.new_Parameter("Iyy5",373.6271);  symbol_numeric  Ixz5 = *sys.new_Parameter("Ixz5",0.0);
    symbol_numeric  Izz5 = *sys.new_Parameter("Izz5",373.6271);  symbol_numeric  Iyz5 = *sys.new_Parameter("Iyz5",0.0);

    // Dynamic Parameters of Bar 6
    symbol_numeric  m6 = *sys.new_Parameter("m6",6776.23246);  symbol_numeric  mx6 = *sys.new_Parameter("mx6",14949.72406);
    symbol_numeric  my6 = *sys.new_Parameter("my6",0.0);    symbol_numeric  mz6 = *sys.new_Parameter("mz6",0.0);
    symbol_numeric  Ixx6 = *sys.new_Parameter("Ixx6",960);  symbol_numeric  Ixy6 = *sys.new_Parameter("Ixy6",0.0);
    symbol_numeric  Iyy6 = *sys.new_Parameter("Iyy6",6083.09722);  symbol_numeric  Ixz6 = *sys.new_Parameter("Ixz6",0.0);
    symbol_numeric  Izz6 = *sys.new_Parameter("Izz6",6083.09722);  symbol_numeric  Iyz6 = *sys.new_Parameter("Iyz6",0.0);

    // Dynamic Parameters of Bar 7
    symbol_numeric  m7 = *sys.new_Parameter("m7",29920);  symbol_numeric  mx7 = *sys.new_Parameter("mx7",66010);
    symbol_numeric  my7 = *sys.new_Parameter("my7",0.0);    symbol_numeric  mz7 = *sys.new_Parameter("mz7",0.0);
    symbol_numeric  Ixx7 = *sys.new_Parameter("Ixx7",13830);  symbol_numeric  Ixy7 = *sys.new_Parameter("Ixy7",0.0);
    symbol_numeric  Iyy7 = *sys.new_Parameter("Iyy7",177300);  symbol_numeric  Ixz7 = *sys.new_Parameter("Ixz7",0.0);
    symbol_numeric  Izz7 = *sys.new_Parameter("Izz7",177300);  symbol_numeric  Iyz7 = *sys.new_Parameter("Iyz7",0.0);


    // Dynamic Parameters of Sun (GearBox)
    symbol_numeric  mSun = *sys.new_Parameter("mSun",4251.4);  symbol_numeric  mxSun = *sys.new_Parameter("mxSun",1505.7);
    symbol_numeric  mySun = *sys.new_Parameter("mySun",0.0);    symbol_numeric  mzSun = *sys.new_Parameter("mzSun",0.0);
    symbol_numeric  IxxSun = *sys.new_Parameter("IxxSun",1153.7);  symbol_numeric  IxySun = *sys.new_Parameter("IxySun",0.0);
    symbol_numeric  IyySun = *sys.new_Parameter("IyySun",1246);  symbol_numeric  IxzSun = *sys.new_Parameter("IxzSun",0.0);
    symbol_numeric  IzzSun = *sys.new_Parameter("IzzSun",1246);  symbol_numeric  IyzSun = *sys.new_Parameter("IyzSun",0.0);

    // Dynamic Parameters of the Planets (GearBox)
    symbol_numeric  mPla1 = *sys.new_Parameter("mPla1",220.9368);  symbol_numeric  mxPla1 = *sys.new_Parameter("mxPla1",0.0);
    symbol_numeric  myPla1 = *sys.new_Parameter("myPla1",0.0);    symbol_numeric  mzPla1 = *sys.new_Parameter("mzPla1",0.0);
    symbol_numeric  IxxPla1 = *sys.new_Parameter("IxxPla1",17.6402);  symbol_numeric  IxyPla1 = *sys.new_Parameter("IxyPla1",0.0);
    symbol_numeric  IyyPla1 = *sys.new_Parameter("IyyPla1",10.4573);  symbol_numeric  IxzPla1 = *sys.new_Parameter("IxzPla1",0.0);
    symbol_numeric  IzzPla1 = *sys.new_Parameter("IzzPla1",10.4573);  symbol_numeric  IyzPla1 = *sys.new_Parameter("IyzPla1",0.0);

    symbol_numeric  mPla2 = *sys.new_Parameter("mPla2",220.9368);  symbol_numeric  mxPla2 = *sys.new_Parameter("mxPla2",0.0);
    symbol_numeric  myPla2 = *sys.new_Parameter("myPla2",0.0);    symbol_numeric  mzPla2 = *sys.new_Parameter("mzPla2",0.0);
    symbol_numeric  IxxPla2 = *sys.new_Parameter("IxxPla2",17.6402);  symbol_numeric  IxyPla2 = *sys.new_Parameter("IxyPla2",0.0);
    symbol_numeric  IyyPla2 = *sys.new_Parameter("IyyPla2",10.4573);  symbol_numeric  IxzPla2 = *sys.new_Parameter("IxzPla2",0.0);
    symbol_numeric  IzzPla2 = *sys.new_Parameter("IzzPla2",10.4573);  symbol_numeric  IyzPla2 = *sys.new_Parameter("IyzPla2",0.0);
    
    symbol_numeric  mPla3 = *sys.new_Parameter("mPla3",220.9368);  symbol_numeric  mxPla3 = *sys.new_Parameter("mxPla3",0.0);
    symbol_numeric  myPla3 = *sys.new_Parameter("myPla3",0.0);    symbol_numeric  mzPla3 = *sys.new_Parameter("mzPla3",0.0);
    symbol_numeric  IxxPla3 = *sys.new_Parameter("IxxPla3",17.6402);  symbol_numeric  IxyPla3 = *sys.new_Parameter("IxyPla3",0.0);
    symbol_numeric  IyyPla3 = *sys.new_Parameter("IyyPla3",10.4573);  symbol_numeric  IxzPla3 = *sys.new_Parameter("IxzPla3",0.0);
    symbol_numeric  IzzPla3 = *sys.new_Parameter("IzzPla3",10.4573);  symbol_numeric  IyzPla3 = *sys.new_Parameter("IyzPla3",0.0);

    // Dynamic Parameters of the Second Stage (GearBox)
    symbol_numeric  mSecSt = *sys.new_Parameter("mSecSt",994.0628);  symbol_numeric  mxSecSt = *sys.new_Parameter("mxSecSt",231.2036);
    symbol_numeric  mySecSt = *sys.new_Parameter("mySecSt",0.0);    symbol_numeric  mzSecSt = *sys.new_Parameter("mzSecSt",0.0);
    symbol_numeric  IxxSecSt = *sys.new_Parameter("IxxSecSt",71.7547);  symbol_numeric  IxySecSt = *sys.new_Parameter("IxySecSt",0.0);
    symbol_numeric  IyySecSt = *sys.new_Parameter("IyySecSt",107.2812);  symbol_numeric  IxzSecSt = *sys.new_Parameter("IxzSecSt",0.0);
    symbol_numeric  IzzSecSt = *sys.new_Parameter("IzzSecSt",107.2812);  symbol_numeric  IyzSecSt = *sys.new_Parameter("IyzSecSt",0.0);
...
// ------------------------------------------------------------------------------------------------------------------------
    ...
    // symbol_numeric KFBearing2x = *sys.new_Input("KFBearing2x",1000.0);
    // symbol_numeric KFBearing2y = *sys.new_Input("KFBearing2y",1000.0);
    // symbol_numeric KFBearing2z = *sys.new_Input("KFBearing2z",1000.0);
...
// ------------------------------------------------------------------------------------------------------------------------
    ...
    symbol_numeric KFGBBearing4x = *sys.new_Input("KFGBBearing4x",1000.0);
    symbol_numeric KFGBBearing4y = *sys.new_Input("KFGBBearing4y",1000.0);
    symbol_numeric KFGBBearing4z = *sys.new_Input("KFGBBearing4z",1000.0);
...
// --------------------------------------------------------------------------------
    ...
    // symbol_numeric KFBearing3x = *sys.new_Input("KFBearing3x",1000.0);
    // symbol_numeric KFBearing3y = *sys.new_Input("KFBearing3y",1000.0);
    // symbol_numeric KFBearing3z = *sys.new_Input("KFBearing3z",1000.0);
...
// --------------------------------------------------------------------------------
    ...
    symbol_numeric KFGBBearing9x = *sys.new_Input("KFGBBearing9x",1000.0);
    symbol_numeric KFGBBearing9y = *sys.new_Input("KFGBBearing9y",1000.0);
    symbol_numeric KFGBBearing9z = *sys.new_Input("KFGBBearing9z",1000.0);
...
// --------------------------------------------------------------------------------
    ...
    symbol_numeric KFGBBearing5x = *sys.new_Input("KFGBBearing5x",1000.0);
    symbol_numeric KFGBBearing5y = *sys.new_Input("KFGBBearing5y",1000.0);
    symbol_numeric KFGBBearing5z = *sys.new_Input("KFGBBearing5z",1000.0);
...
// --------------------------------------------------------------------------------
    ...
    symbol_numeric KFGBBearing6x = *sys.new_Input("KFGBBearing6x",1000.0);
    symbol_numeric KFGBBearing6y = *sys.new_Input("KFGBBearing6y",1000.0);
    symbol_numeric KFGBBearing6z = *sys.new_Input("KFGBBearing6z",1000.0);
...
// --------------------------------------------------------------------------------
    ...
    symbol_numeric KFGBBearing1x = *sys.new_Input("KFGBBearing1x",1000.0);
    symbol_numeric KFGBBearing1y = *sys.new_Input("KFGBBearing1y",1000.0);
    symbol_numeric KFGBBearing1z = *sys.new_Input("KFGBBearing1z",1000.0);
...
// --------------------------------------------------------------------------------
    ...
    symbol_numeric KFGBBearing2x = *sys.new_Input("KFGBBearing2x",1000.0);
    symbol_numeric KFGBBearing2y = *sys.new_Input("KFGBBearing2y",1000.0);
    symbol_numeric KFGBBearing2z = *sys.new_Input("KFGBBearing2z",1000.0);
...
// --------------------------------------------------------------------------------
    ...
    symbol_numeric KFGBBearing3x = *sys.new_Input("KFGBBearing3x",1000.0);
    symbol_numeric KFGBBearing3y = *sys.new_Input("KFGBBearing3y",1000.0);
    symbol_numeric KFGBBearing3z = *sys.new_Input("KFGBBearing3z",1000.0);
...
// --------------------------------------------------------------------------------
    ...
    symbol_numeric KFGBBearing7x = *sys.new_Input("KFGBBearing7x",1000.0);
    symbol_numeric KFGBBearing7y = *sys.new_Input("KFGBBearing7y",1000.0);
    symbol_numeric KFGBBearing7z = *sys.new_Input("KFGBBearing7z",1000.0);
...
// --------------------------------------------------------------------------------
// GBBearing8 STIFFNESS-DAMPING (SOLIDS 4-SecSt) 
// --------------------------------------------------------------------------------
    // Translational stiffness of the coupling between GEARBOX and SecSt
    ...
    symbol_numeric KFGBBearing8x = *sys.new_Input("KFGBBearing8x",1000.0);
    symbol_numeric KFGBBearing8y = *sys.new_Input("KFGBBearing8y",1000.0);
    symbol_numeric KFGBBearing8z = *sys.new_Input("KFGBBearing8z",1000.0);
...
// --------------------------------------------------------------------------------
    ...
    symbol_numeric TGENx = *sys.new_Input("TGENx",0.0); //Tgennom= (3e+04)/pi=9549.29658551372 N/m

    // Commanded speed 
    symbol_numeric commanded_speed =	*sys.new_Input("commanded_speed",1.62457); //1500/Ratio*2*pi/60
...
/*
    symbol_numeric modulus_1 = *sys.new_Parameter("modulus_1",0.008);
    symbol_numeric modulus_2 = *sys.new_Parameter("modulus_2",0.0045);


    ex R_Pla = Z_Pla*modulus_1/2.0;
    ex R_Sun = Z_Sun*modulus_1/2.0;
    ex R_Ring = Z_Ring*modulus_1/2.0;
    ex R_SecSt_1 = Z_SecSt1*modulus_1/2.0;
    ex R_SecSt_2 = Z_SecSt2*modulus_1/2.0;
    ex L_Sun_SecSt = R_SecSt_1+R_SecSt_2;
    ex R_ThiSt_1 = Z_ThiSt1*modulus_2/2.0;
    ex R_ThiSt_2 = Z_ThiSt2*modulus_2/2.0;
    ex L_SecSt_ThiSt = R_ThiSt_1+R_ThiSt_2;
*/
...
/*    Matrix Output(Coordinates.size()+Velocities.size()+Joint_Unknowns.size()+3,1);
   ...
   // Output(4,0) = a5/a3; 
   //Output(5,0) = *da5/(*da3); 
    //GBBearing1 Hub-Gearbox Planet1
    Output(0,0) = GBBearing1S3_GBBearing1Pla1 (0,0);
    Output(1,0) = GBBearing1S3_GBBearing1Pla1 (1,0);
    Output(2,0) = GBBearing1S3_GBBearing1Pla1 (2,0);
    //GBBearing2 Hub-Gearbox Planet2
    Output(3,0) = GBBearing2S3_GBBearing2Pla2 (0,0);
    Output(4,0) = GBBearing2S3_GBBearing2Pla2 (1,0);
    Output(5,0) = GBBearing2S3_GBBearing2Pla2 (2,0);
    //GBBearing3 Hub-Gearbox Planet3
    Output(6,0) = GBBearing3S3_GBBearing3Pla3 (0,0);
    Output(7,0) = GBBearing3S3_GBBearing3Pla3 (1,0);
    Output(8,0) = GBBearing3S3_GBBearing3Pla3 (2,0);
    //GBBearing4 Hub-Gearbox
    Output(9,0) = GBBearing4S3_GBBearing4S4 (0,0);
    Output(10,0) = GBBearing4S3_GBBearing4S4 (1,0);
    Output(11,0) = GBBearing4S3_GBBearing4S4 (2,0);
    //GBBearing5 Gearbox-Sun pinion
    Output(12,0) = GBBearing5S4_GBBearing5Sun (0,0);
    Output(13,0) = GBBearing5S4_GBBearing5Sun(1,0);
    Output(14,0) = GBBearing5S4_GBBearing5Sun (2,0);
    //GBBearing6 Gearbox-Sun 
    Output(15,0) = GBBearing6S4_GBBearing6Sun (0,0);
    Output(16,0) = GBBearing6S4_GBBearing6Sun(1,0);
    Output(17,0) = GBBearing6S4_GBBearing6Sun (2,0);
    //GBBearing7
    Output(18,0) = GBBearing7S4_GBBearing7SecSt (0,0);
    Output(19,0) = GBBearing7S4_GBBearing7SecSt(1,0);
    Output(20,0) = GBBearing7S4_GBBearing7SecSt (2,0);
    //GBBearing8 Gearbox-SecondStage
    Output(21,0) = GBBearing8S4_GBBearing8SecSt (0,0);  
    Output(22,0) = GBBearing8S4_GBBearing8SecSt (1,0);
    Output(23,0) = GBBearing8S4_GBBearing8SecSt (2,0);
    //GBBearing9 Gearbox-HSS
    Output(24,0) = GBBearing9S4_GBBearing9S5 (0,0);   
    Output(25,0) = GBBearing9S4_GBBearing9S5 (1,0);
    Output(26,0) = GBBearing9S4_GBBearing9S5 (2,0);
    //GBearing2
    Output(27,0) = Bearing2S3_Bearing2S4 (0,0); 
    Output(28,0) = Bearing2S3_Bearing2S4 (1,0); 
    Output(29,0) = Bearing2S3_Bearing2S4 (2,0);
     //GBearing3
    Output(30,0) = Bearing3S4_Bearing3S5 (0,0); 
    Output(31,0) = Bearing3S4_Bearing3S5 (1,0); 
    Output(32,0) = Bearing3S4_Bearing3S5 (2,0);
