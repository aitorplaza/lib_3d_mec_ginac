...
Phi_d();
  ...
  #if defined(INV) 
  double Buffer[2*MAX(Phi_d_n_cols,n_accel_indep)*MAX(Phi_d_n_cols,n_accel_indep)];
  #elif defined(TRI)
  double Buffer[2*Phi_d_n_cols*Phi_d_n_cols];
  double BufferLUs[Phi_d_n_cols];
  double BufferSLUs[ n_coord_indep];
  double BufferSLULU[ n_coord_indep * n_coord_indep];
  double BufferSLUy[ n_coord_indep];
  int BufferSLUo[ n_coord_indep ];
	#endif

	#ifdef INV
	/* Calculate A and b and e */
	Mat_Inv(Phi_d_n_cols, _Phi_d, _Inv_Phi_d, Buffer);
	...
	#ifdef TRI
	/* Calculate A and b and e */
	  lu(Phi_d_n_cols, _Phi_d, LU, o,BufferLUs, niter);
  /*Phi_z();*/
	...
	_Beta=Beta();
	forward_substitution_piv(Phi_d_n_cols, LU, o, _Beta, y);
	backward_substitution_piv(Phi_d_n_cols, LU, o, y, e);
	#endif
...
while (error > TRAPEZOIDAL_TOLERANCE){
	...
	#ifdef INV
		Mat_Inv( n_accel_indep, fz_z, INV_fz_z, Buffer);
		Mat_Vect_Mult(n_coord_indep, n_coord_indep, INV_fz_z, fz, -1, delta_z);
	#endif

	#ifdef TRI
		solve_lin_sys_lu(n_coord_indep, fz_z, fz, delta_z,BufferSLUs, BufferSLULU, BufferSLUy, BufferSLUo,niter);
