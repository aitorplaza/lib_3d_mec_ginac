...
extern double daxpy(int * n_coord, double * dmone, double *qaux, int * ione, double * edq, int * ione2);
  ...
  #ifdef COORD_DEP
          extern void Initial_position_velocity_q (double * q, double *dq);    
	  #ifdef EULER
	    extern void one_step_euler_q (double delta_t, double * q, double *dq);
	  #endif
	  #ifdef TRAPEZOIDAL
            extern void one_step_trapezoidal_q (double delta_t, double * q, double *dq);
	  #endif
	  #ifdef I3AL
	    extern void one_step_i3al_q (double delta_t, double * q, double *dq);
	  #endif
  #endif

  #ifdef COORD_IND
          extern void Initial_position_velocity_z (double * q, double *dq);    
	  #ifdef EULER
	    extern void one_step_euler_z (double delta_t, double * q, double *dq);
	  #endif
          #ifdef TRAPEZOIDAL
            extern void one_step_trapezoidal_z (double delta_t, double * q, double *dq);
	  #endif
	  #ifdef I3AL
	    extern void one_step_i3al_z (double delta_t, double * q, double *dq);
	  #endif
  #endif
