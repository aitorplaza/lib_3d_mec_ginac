...
cout << "*    v 4.0 with splines                        *" << endl;
cout << "*    Aitor Plaza Nov 2013                      *" << endl;
cout << "************************************************" << endl;
cout << " " << endl;
system("date");
cout << " " << endl;
...
// ************************************************************************************************
    ...
    symbol_numeric u_W_R = *sys.new_Coordinate("u_W_R","du_W_R","ddu_W_R",0.0 ,0.0,0.0);
    symbol_numeric *du_W_R=sys.get_Velocity("du_W_R");
    symbol_numeric *ddu_W_R=sys.get_Acceleration("ddu_W_R");
    ...
    symbol_numeric u_W_L = *sys.new_Coordinate("u_W_L","du_W_L","ddu_W_L",0.0 ,0.0,0.0);
    symbol_numeric *du_W_L=sys.get_Velocity("du_W_L");
    symbol_numeric *ddu_W_L=sys.get_Acceleration("ddu_W_L");
    
    symbol_numeric u_R_R = *sys.new_Coordinate("u_R_R","du_R_R","ddu_R_R",0.0,0.0,0.0);
    symbol_numeric *du_R_R=sys.get_Velocity("du_R_R");
    symbol_numeric *ddu_R_R=sys.get_Acceleration("ddu_R_R");
    ...
    symbol_numeric u_R_L = *sys.new_Coordinate("u_R_L","du_R_L","ddu_R_L",0.0,0.0,0.0);
    symbol_numeric *du_R_L=sys.get_Velocity("du_R_L");
    symbol_numeric *ddu_R_L=sys.get_Acceleration("ddu_R_L");
    ...
    //spline coefficients wheel
    symbol_numeric awL = *sys.new_Parameter("awL",-0.005980940917141e+4);
    symbol_numeric bwL = *sys.new_Parameter("bwL",-0.000002379249587e+4);
    symbol_numeric cwL = *sys.new_Parameter("cwL",-0.000000893613412e+4);
    symbol_numeric dwL = *sys.new_Parameter("dwL",-0.000050000000000e+4);
    symbol_numeric xwL = *sys.new_Parameter("xwL",0.01);
    
    symbol_numeric awR = *sys.new_Parameter("awR",0.006456790834618e+4);
    symbol_numeric bwR = *sys.new_Parameter("bwR",-0.000002379249587e+4);
    symbol_numeric cwR = *sys.new_Parameter("cwR",0.000000893613412e+4);
    symbol_numeric dwR = *sys.new_Parameter("dwR",-0.000050000000000e+4);
    symbol_numeric xwR = *sys.new_Parameter("xwR",0.01);


    //~ ex f_r=-R;
    ex swR = u_W_R - xwR;
    ex splineWR =   dwR + swR * ( cwR + swR * (bwR + swR * awR ) );
    Vector3D fWHSR = *sys.new_Vector3D("fWHSR",-splineWR*sin(theta_W_R),u_W_R,splineWR*cos(theta_W_R),"B_WHSa");
    Vector3D r_WHSR = sys.Position_Vector("O","P_WHSR")+fWHSR;
    Vector3D t1_WHSR= sys.diff ( r_WHSR , theta_W_R);
    Vector3D t2_WHSR= sys.diff ( r_WHSR , u_W_R);
    Vector3D n_WHSR = t1_WHSR^t2_WHSR;


    ex swL = u_W_L - xwL;
    ex splineWL =   dwL + swL * ( cwL + swL * (bwL + swL * awL ) );
    Vector3D fWHSL = *sys.new_Vector3D("fWHSL",-splineWL*sin(theta_W_L),u_W_L,splineWL*cos(theta_W_L),"B_WHSa");
    Vector3D r_WHSL = sys.Position_Vector("O","P_WHSL")+fWHSL;
    Vector3D t1_WHSL= sys.diff ( r_WHSL , theta_W_L);
    Vector3D t2_WHSL= sys.diff ( r_WHSL , u_W_L);
    Vector3D n_WHSL = t1_WHSL^t2_WHSL;
    
    //spline rail
    symbol_numeric arL = *sys.new_Parameter("arL",0.066403345669996e+3);
    symbol_numeric brL = *sys.new_Parameter("brL",0.001998816728350e+3);
    symbol_numeric crL = *sys.new_Parameter("crL",0.000000000000000e+3);
    symbol_numeric drL = *sys.new_Parameter("drL",0.000159000000000e+3);
    symbol_numeric xrL = *sys.new_Parameter("xrL",0.005);

    symbol_numeric arR = *sys.new_Parameter("arR",0.066403345669996e+3);
    symbol_numeric brR = *sys.new_Parameter("brR",-0.001998816728350e+3);
    symbol_numeric crR = *sys.new_Parameter("crR",0.000000000000000e+3);
    symbol_numeric drR = *sys.new_Parameter("drR",0.000159000000000e+3);
    symbol_numeric xrR = *sys.new_Parameter("xrR",0.005);

    ex srR = u_R_R - xrR;
    ex splineRR =   drR + srR * ( crR + srR * (brR + srR * arR ) );
    Vector3D f_RAILR = *sys.new_Vector3D("r_RAILR",x_R_R,u_R_R,splineRR,"B_Rail"); //SPLINE
    Vector3D r_RAILR = sys.Position_Vector("O","P_RailR")+f_RAILR;
    Vector3D t1_RAILR = sys.diff ( r_RAILR , x_R_R);
    Vector3D t2_RAILR = sys.diff ( r_RAILR , u_R_R);
    Vector3D n_RAILR = t1_RAILR^t2_RAILR;

    ex srL = u_R_L - xrL;
    ex splineRL =   drL + srL * ( crL + srL * (brL + srL * arL ) );
    Vector3D f_RAILL = *sys.new_Vector3D("r_RAILL",x_R_L,u_R_L,splineRL,"B_Rail"); //SPLINE
    Vector3D r_RAILL = sys.Position_Vector("O","P_RailL")+f_RAILL; 
    Vector3D t1_RAILL = sys.diff ( r_RAILL , x_R_L);
    Vector3D t2_RAILL = sys.diff ( r_RAILL , u_R_L);
...
Drawing3D * D_n_RAILL   = sys.new_Drawing3D("D_n_RAILL",  &u_nRAILL, P_con_L,1,0,1,1.0);
Drawing3D * D_u_t1RAILL = sys.new_Drawing3D("D_u_t1RAILL",&u_t1RAILL,P_con_L,1,0,1,1.0);
Drawing3D * D_u_t2RAILL = sys.new_Drawing3D("D_u_t2RAILL",&u_t2RAILL,P_con_L,1,0,1,1.0);
Drawing3D * D_n_RAILR   = sys.new_Drawing3D("D_n_RAILR",  &u_nRAILR, P_con_R,1,0,1,1.0);
Drawing3D * D_u_t1RAILR = sys.new_Drawing3D("D_u_t1RAILR",&u_t1RAILR,P_con_R,1,0,1,1.0);
Drawing3D * D_u_t2RAILR = sys.new_Drawing3D("D_u_t2RAILR",&u_t2RAILR,P_con_R,1,0,1,1.0);
// ************************************************************************************************
// 	Dynamic Equations 
// ************************************************************************************************
...
// ************************************************************************************************
    ...
    Vector3D r_PWHSR = sys.Position_Vector("O","P_WHSR");
    Vector3D r_PWHSL = sys.Position_Vector("O","P_WHSL");
    ...
    Output(6,0) = r_PWHSR(1,0);
    Output(7,0) = r_PWHSR(2,0);
    Output(8,0) = r_PWHSL(1,0);
    Output(9,0) = r_PWHSL(2,0);
