...
int i,j,k, iter, ti , mrank, niter=0;
...
int p_vector[dPhiInit_dq_n_rows], q_vector[dPhiInit_dq_n_cols];
		...
		double aux_Phi_q_T[ M_n_rows * dPhi_dq_n_rows ];
...
/* Coord z */
  ...
  double LU[MAX(Phi_q_n_rows,dPhi_dq_n_rows)*MAX(Phi_q_n_cols,dPhi_dq_n_cols)],LUpq[MAX(Phi_q_n_rows,dPhi_dq_n_rows)*MAX(Phi_q_n_cols,dPhi_dq_n_cols)], Y[MAX(Phi_q_n_rows,dPhi_dq_n_rows) * MAX(Phi_q_n_cols,dPhi_dq_n_cols)], y[MAX(Phi_q_n_cols,dPhi_dq_n_cols)];
  ...
  double AUX_L_dep[1*n_vel_dep],AUX_L_indep[1*n_vel_indep],b_aux[n_vel_dep*1], yL[n_vel_dep*1];
#endif
/* ********************************************************************************************* */
/* ********************************************************************************************* */
#ifdef TRAPEZOIDAL
