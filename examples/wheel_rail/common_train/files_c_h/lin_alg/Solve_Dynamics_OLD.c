...
perm_M[i]=perm_vel[i+n_vel_dep];
}
for (i = 0; i< n_vel_dep ; i++){
perm_M[i+n_vel_indep]=perm_vel[i];
}
...
for (i=0; i<n_vel_indep; i++){
	for (j=0; j< n_vel_indep; j++){
		...
		for	(k=0; k<dPhi_dq_n_rows; k++){
			AUX_M_indep_indep_1[i+j*n_vel_indep] =  _M[PERM(i,k,i_ini, j_ini,M_n_rows, perm_M)] * A[k+j*dPhi_dq_n_rows] + AUX_M_indep_indep_1[i+j*n_vel_indep];
			}
		}
	}
/*A'*Mdd*/
...
for (i=0; i<n_vel_indep; i++){
	for (j=0; j< dPhi_dq_n_rows; j++){
		...
		for	(k=0; k<dPhi_dq_n_rows; k++){
  			AUX_M_indep_dep_1[i+j*n_vel_indep] =   A[k+i*dPhi_dq_n_rows] * _M[PERM(k,j,i_ini, j_ini,M_n_rows, perm_M)]  + AUX_M_indep_dep_1[i+j*n_vel_indep];
			}
		}
	}
...
/*AUX_V_indep_1=A'* Qd*/
 for (i=0; i<n_vel_indep; i++){
	AUX_V_indep_1[i] = 0.0;
	for (j=0; j<dPhi_dq_n_rows; j++){
		AUX_V_indep_1[i] = A[j+i*dPhi_dq_n_rows] * _Q[perm_M[j+n_vel_indep]] + AUX_V_indep_1[i];
		}
	}
...
j_ini=0;
	for (i = 0; i < n_vel_indep; i++){
		for(j = 0; j < n_vel_indep; j++){	
		_MZ[i + j * n_vel_indep]= _M[PERM(i,j,i_ini, j_ini, M_n_rows, perm_M)] + AUX_M_indep_indep_1[j + i * n_vel_indep] + AUX_M_indep_indep_1[i + j * n_vel_indep] + AUX_M_indep_indep_2[i + j * n_vel_indep]; 
		}
	}
/*Mzd+(A'*Mdd)*/
i_ini=0;
j_ini=n_vel_indep;     
	for (i = 0; i < n_vel_indep; i++){
		for(j = 0; j < dPhi_dq_n_rows; j++){
		AUX_M_indep_dep_2[i + j * n_vel_indep] = _M[PERM(i,j,i_ini, j_ini,M_n_rows, perm_M)] + AUX_M_indep_dep_1[i + j * n_vel_indep];
		}
	}
Mat_Vect_Mult(n_vel_indep, dPhi_dq_n_rows, AUX_M_indep_dep_2, b, +1, AUX_V_indep_2);	
/*_QZ = Qz + A'* Qd - (Mzd+(A'*Mdd))*b*/
	for (i = 0; i < n_vel_indep; i++){
		_QZ[i] = _Q[perm_M[i]] + AUX_V_indep_1[i] - AUX_V_indep_2[i];
	}
/* Solve MZ * ddz = QZ */

solve_lin_sys_ldl(n_vel_indep,_MZ , _QZ, aux_ddq, AUX_M_indep_indep_1, AUX_M_indep_indep_2 , AUX_V_indep_1 , AUX_V_indep_2);

/* Calculate  ddd = A * ddz + b -> C.4 Equation DSM*/
Mat_Vect_Mult(dPhi_dq_n_rows, n_vel_indep, A, aux_ddq, +1, &aux_ddq[n_vel_indep]);
for (i = 0; i < dPhi_dq_n_rows; i++){ aux_ddq[n_vel_indep + i] = aux_ddq[n_vel_indep + i] + b[i]; }
for (i = 0; i < dPhi_dq_n_cols; i++){ ddq[perm_M[i]]=aux_ddq[i]; }
...
for (i=0; i<n_vel_dep; i++){
	...
	for (j=0; j<n_vel_dep; j++){
		AUX_L_dep[i] = AUX_L_dep[i] + _M[PERM(i,j,i_ini, j_ini,M_n_rows, perm_M)] * ddq[perm_M[j+n_vel_indep]] ;
		}
	}
/* Mdz · ddz */
i_ini=n_vel_indep;
...
for (i=0; i<n_vel_dep; i++){
	AUX_L_indep[i] = 0.0;
	for (j=0; j<n_vel_indep; j++){
		AUX_L_indep[i] = AUX_L_indep[i] + _M[PERM(i,j,i_ini, j_ini,M_n_rows, perm_M)] * ddq[perm_M[j]] ;
		}
	}
/* Qd - Mdd * ddd - Mdz * ddz */
//for (i = 0; i < n_vel_dep; i++){aux_qd[i] = 0.0;}
for (i = 0; i < n_vel_dep; i++){
	aux_qd[i] = _Q[perm_M[n_vel_indep+i]] - AUX_L_dep[i] - AUX_L_indep[i];
}

forward_substitution_LUpq_T_pq_q (n_vel_dep,n_vel_dep, _dPhi_dq, p_vector, perm_vel, aux_qd, yL);
