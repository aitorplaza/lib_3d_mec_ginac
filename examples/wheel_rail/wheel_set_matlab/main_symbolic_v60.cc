...
cout << "*    v 5.0 with splines and Kalker             *" << endl;
...
// ************************************************************************************************
    ...
    symbol_numeric a_rail = *sys.new_Parameter("a_rail",0.05);
...
// ************************************************************************************************
    ...
    symbol_numeric thetaW_R = *sys.new_Coordinate("thetaW_R","dthetaW_R","ddthetaW_R",0.0,0.0,0.0);
    symbol_numeric uW_R = *sys.new_Coordinate("uW_R","duW_R","dduW_R",-0.0,0.0,0.0);
    ...
    symbol_numeric uR_R = *sys.new_Coordinate("uR_R","duR_R","dduR_R", -0.0 ,0.0,0.0);
    symbol_numeric xR_R = *sys.new_Coordinate("xR_R","dxR_R","ddxR_R", 0.0 ,0.0,0.0);

    //~ symbol_numeric aR_R = *sys.new_Parameter("aR_R",0.066403345669996e+3);//spline's coefficients right wheel
    ...
    symbol_numeric aR_R = *sys.new_Parameter("aR_R",0.0);//spline's coefficients right wheel
    ...
    ex sR_R = uR_R - lR_R;
    ex splineR_R   = dR_R + sR_R * ( cR_R + sR_R * (bR_R + sR_R * aR_R ) );
    
    Vector3D fR_R  = *sys.new_Vector3D("fR_R",xR_R,uR_R,splineR_R,"B_RailR");
    Vector3D rR_R  = sys.Position_Vector("O","P_RailR")+fR_R;
    Vector3D t1R_R = sys.diff ( rR_R , xR_R);
 ...
 // ************************************************************************************************
    ...
    symbol_numeric thetaW_L = *sys.new_Coordinate("thetaW_L","dthetaW_L","ddthetaW_L",0.0,0.0,0.0);
    symbol_numeric uW_L = *sys.new_Coordinate("uW_L","duW_L","dduW_L",0.0,0.0,0.0);
    ...
    symbol_numeric uR_L = *sys.new_Coordinate("uR_L","duR_L","dduR_L", 0.0 ,0.0,0.0);
    symbol_numeric xR_L = *sys.new_Coordinate("xR_L","dxR_L","ddxR_L", 0.0 ,0.0,0.0);
    ...
    symbol_numeric aR_L = *sys.new_Parameter("aR_L",0.0);
    ...
    ex sR_L = uR_L - lR_L;
    ex splineR_L   =   dR_L + sR_L * ( cR_L + sR_L * (bR_L + sR_L * aR_L ) );
    Vector3D fR_L  = *sys.new_Vector3D("fR_L",xR_L,uR_L,splineR_L,"B_RailL");

    Vector3D rR_L  = sys.Position_Vector("O","P_RailL")+fR_L;
    Vector3D t1R_L = sys.diff ( rR_L , xR_L);
...
// ************************************************************************************************
    ...
    symbol_numeric nu_poisson = *sys.new_Parameter("nu_poisson",0.3);     // Steel
    symbol_numeric G_elastic = *sys.new_Parameter("G_elastic",80.7e+9);   // G = E/(2*(1+nu)); % MPa
...
// ************************************************************************************************   
    ...
    //~ Matrix CKALKER = dQ_dKFR * CKR*pow(numeric(1,2)*(VCW_R.get_module() + (OM_WHS ^ fW_R).get_module()),-1) * sys.jacobian(SlipR.transpose(),dq) 
                   //~ + dQ_dKFL * CKL*pow(numeric(1,2)*(VCW_L.get_module() + (OM_WHS ^ fW_L).get_module()),-1) * sys.jacobian(SlipL.transpose(),dq);
    Matrix CKR = dQ_dKFR * D_R*sys.jacobian(SlipR.transpose(),dq);
    Matrix CKL = dQ_dKFL * D_L*sys.jacobian(SlipL.transpose(),dq);                 
              
    cout <<" CKalker Exportng"<<endl;       
    ...
    sys.export_function_MATLAB("CKR", "CKR_",CKR, new_atom_list_CKR, new_exp_list_CKR,"q,dq,time,param");   
    ...
    sys.export_function_MATLAB("CKL", "CKL_",CKL, new_atom_list_CKL, new_exp_list_CKL,"q,dq,time,param");     
...
// sustituir las fuerzas de kalker (inputs) por cero en Q y exportar el nuevo Q (Q2)
    ...
    lst new_atom_list_Q2, new_exp_list_Q2;
    matrix_list_optimize (Qo, new_atom_list_Q2,new_exp_list_Q2);
    sys.export_Matrix_C	("Q2","_Q2",Qo, new_atom_list_Q2, new_exp_list_Q2 ,ORDER);
    sys.export_function_MATLAB("Q2", "Q2_",Qo, new_atom_list_Q2, new_exp_list_Q2,"q,dq,time,param"); 
...
// ************************************************************************************************
    ...
    state = x, y, z, a,b, c,*dx, *dy, *dz, *da,*db, *dc,uW_R,uW_L;    
