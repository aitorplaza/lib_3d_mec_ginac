...
// ************************************************************************************************
	...
	symbol_numeric L_R = *sys.new_Parameter("L_R",2*1.0);
	symbol_numeric LWHS= *sys.new_Parameter("L_WHS",2*1.0);

	//symbol_numeric L_R = *sys.new_Parameter("L_R",2*0.815);
	//symbol_numeric LWHS= *sys.new_Parameter("L_WHS",2*0.815);
...
Drawing3D * VectorVpcL = sys.new_Drawing3D("VectorVpcL",&vel_PcL,Pcon_L);
Drawing3D * VectorVpcR = sys.new_Drawing3D("VectorVpcR",&vel_PcR,Pcon_R);
Drawing3D * PointContactL = sys.new_Drawing3D("PointContactL",Pcon_L,0.03);	
Drawing3D * PointContactR = sys.new_Drawing3D("PointContactR",Pcon_R,0.03);	

        Vector3D F_rozL = (Fx_L* u_t1RAILL + Fy_L * u_t2RAILL);
        Vector3D F_rozR = (Fx_R* u_t1RAILR + Fy_R * u_t2RAILR);

//Drawing3D * D_P_WHSL = sys.new_Drawing3D("D_P_WHSL",P_WHSL,0.03);	
//Drawing3D * D_P_WHSR = sys.new_Drawing3D("D_P_WHSR",P_WHSR,0.03);
//Drawing3D * D_fWHSL = sys.new_Drawing3D("D_fWHSL",&fWHSL,P_WHSR);
//Drawing3D * D_fWHSR = sys.new_Drawing3D("D_fWHSR",&fWHSR,P_WHSR);

Drawing3D * D_F_rozL = sys.new_Drawing3D("D_F_rozL",&F_rozL,Pcon_L,0,1,1,0.6);
Drawing3D * D_F_rozR = sys.new_Drawing3D("D_F_rozR",&F_rozR,Pcon_R,0,1,1,0.6);
...
D_F_rozR->set_scale(0.01);
  	...
  	Vector3D M_rozL  = Mz_L * u_nRAILL;//Se ha de aplicar en el centro de la reudas
  	Vector3D M_rozR  = Mz_R * u_nRAILR;
  	Vector3D M_roz  = *sys.new_Vector3D("M_roz",0,0,0,"B_WHSa");


Drawing3D * D_M_rozL = sys.new_Drawing3D("D_M_rozL",&M_rozL,P_WHS,0,1,1,1.0);
Drawing3D * D_M_rozR = sys.new_Drawing3D("D_M_rozR",&M_rozR,P_WHS,0,1,1,1.0);
...
//Drawing3D * D_u_t2RAILR = sys.new_Drawing3D("D_u_t2RAILR",&u_t2RAILR,Pcon_R,1,0,1,1.0);
	...
	Wrench3D * FrictionL = sys.new_Wrench3D ( "FrictionL", F_rozL, M_roz , Pcon_L, WHS , "Constitutive") ;
	Wrench3D * FrictionR = sys.new_Wrench3D ( "FrictionR", F_rozR, M_roz , Pcon_R, WHS , "Constitutive") ;
