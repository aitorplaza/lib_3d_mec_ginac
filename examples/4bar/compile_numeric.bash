...
MAIN=main_numeric
###################################################################################################
TIME=-DTIME_OFF # Change to TIME_OFF or TIME_ON
###################################################################################################
...
#####################################################################################################################################################
  ...
  if [ "$INTEG" == 1 ];  then
    if [ "$FORM" == 1 ];  then 
      ...
      gcc -DLIN_ALG $TIME $INIT $EULER $Q $DDL $TRI_LDL $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDL one_step_euler.c -lm
    ...
    if [ "$FORM" == 2 ];  then 
      ...
      gcc -DLIN_ALG $TIME $INIT $EULER $Q $DDQ $TRI_LDL $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDQ one_step_euler.c -lm
      ./$MAIN $TTIME $TSTEP $name;
    fi
    if [ "$FORM" == 3 ];  then 
      ...
      gcc -DLIN_ALG $TIME $INIT $EULER $Z $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Z one_step_euler.c -lm
      ./$MAIN $TTIME $TSTEP $name;		
    fi
  fi
  ...
  if [ "$INTEG" == 2 ];  then
    if [ "$FORM" == 1 ];  then 
      ...
      gcc -DLIN_ALG $TIME $INIT $TRAPEZOIDAL $Q $DDL $TRI_LDL $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDL one_step_trapezoidal.c -lm
      ./$MAIN $TTIME $TSTEP $name;
    fi
    if [ "$FORM" == 2 ];  then 
      ...
      gcc -DLIN_ALG $TIME $INIT $TRAPEZOIDAL $Q $DDQ $TRI_LDL $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDQ one_step_trapezoidal.c -lm
      ./$MAIN $TTIME $TSTEP $name;
    fi
    if [ "$FORM" == 3 ];  then 
      ...
      gcc -DLIN_ALG $TIME $INIT $TRAPEZOIDAL $Z $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Z one_step_trapezoidal.c -lm
      ./$MAIN $TTIME $TSTEP $name;		
    fi
  fi
...
#####################################################################################################################################################
  ...
  if [ "$INTEG" == 1 ];  then
    ...
    if [ "$FORM" == 1 ];  then 
      ...
      gcc -DLIN_ALG $TIME $INIT $EULER $Q $DDL $TRI_LU $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDL one_step_euler.c -lm
      ./$MAIN $TTIME $TSTEP $name;
    fi
    ...
    if [ "$FORM" == 2 ];  then 
      ...
      gcc -DLIN_ALG $TIME $INIT $EULER $Q $DDQ $TRI_LU $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDQ one_step_euler.c -lm
      ./$MAIN $TTIME $TSTEP $name;
    fi
  ...
  if [ "$INTEG" == 2 ];  then
    ...
    if [ "$FORM" == 1 ];  then 
      ...
      gcc -DLIN_ALG $TIME $INIT $TRAPEZOIDAL $Q $DDL $TRI_LU $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDL one_step_trapezoidal.c -lm
      ./$MAIN $TTIME $TSTEP $name;
    fi
    ...
    if [ "$FORM" == 2 ];  then 
      ...
      gcc -DLIN_ALG $TIME $INIT $TRAPEZOIDAL $Q $DDQ $TRI_LU $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDQ one_step_trapezoidal.c -lm
      ./$MAIN $TTIME $TSTEP $name;
    fi
...
###################################################################################################
  ...
  if [ "$FORM" == 1 ] && [ "$INTEG" == 1 ];  then 
    ...
    gcc -DLIN_ALG $TIME $EULER $Q $DDL $INV $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDL one_step_euler.c -lm
    ./$MAIN $TTIME $TSTEP $name;
  fi
  if [ "$FORM" == 1 ] && [ "$INTEG" == 2 ];  then 
    ...
    gcc -DLIN_ALG $TIME $TRAPEZOIDAL $Q $DDL $INV $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDL one_step_trapezoidal.c -lm
    ./$MAIN $TTIME $TSTEP $name;
  fi
...
if [ "$MATH" == 4 ];  then
  ...
  if [ "$INTEG" == 1 ];  then
    if [ "$FORM" == 1 ];  then
      ...
      gcc  -DLAPACK $TIME $EULER $Q $DDL -O2 -o $MAIN $MAIN.c $COMMON $COORD_Q $DYN_DDL newton_raphson_min_norm.c -lm -lblas -llapack
      ./$MAIN $TTIME $TSTEP $name;
    fi
    if [ "$FORM" == 2 ];  then
  ...
  if [ "$INTEG" == 2 ];  then
    if [ "$FORM" == 1 ];  then
      ...
      gcc  -DLAPACK $TIME $TRAPEZOIDAL $Q $DDL -O2 -o $MAIN $MAIN.c $COMMON $COORD_Q $DYN_DDL newton_raphson_min_norm.c -lm -lblas -llapack
      ./$MAIN $TTIME $TSTEP $name;
    fi
    if [ "$FORM" == 2 ];  then
