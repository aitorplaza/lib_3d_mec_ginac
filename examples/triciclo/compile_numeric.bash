...
echo "----------------------------------------------"
echo " "
echo "Example: Simple Formalism with EULER integrator, and LIN_ALG Math library, Delta_t=0.001 and integration time 10 seconds"
...
TRI=-DTRI #Uses Choleski for symmetric definite positive, LDL for symmetric semi definite positive and LU for Other
###################################################################################################
...
#####################################################################################################################################################
    ...
    name="state_lin_alg_Euler_Simple_form.dat"; rm -f -r ./GENERATED_files_dat/$name
  	gcc -DLIN_ALG -DTIME_OFF $EULER $Q $DDL $TRI $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDL one_step_euler_q.c -lm
	  ./$MAIN $TTIME $TSTEP $name;
		fi

		if [ "$FORM" == 2 ];  then 
		echo "Compiling Model with Udwadia Formalism and EULER integrator, solving with LIN_ALG library";
    name="state_lin_alg_Euler_Udwadia_form.dat"; rm -f -r ./GENERATED_files_dat/$name
  	gcc -DLIN_ALG -DTIME_OFF $EULER $Q $DDQ $TRI $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDQ one_step_euler_q.c -lm
	  ./$MAIN $TTIME $TSTEP $name;
		fi

		if [ "$FORM" == 3 ];  then 
		echo "Compiling Model with Independent Velocity Formalism and EULER integrator, solving with LIN_ALG library";
    name="state_lin_alg_Euler_Indep_Vel_form.dat"; rm -f -r ./GENERATED_files_dat/$name
  	gcc -DLIN_ALG -DTIME_OFF -DROBUST_SOLVER $EULER $Z $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Z one_step_euler_z.c -lm
	  ./$MAIN $TTIME $TSTEP $name;		
    fi
    ...
    name="state_lin_alg_Trapezoidal_Simple_form.dat"; rm -f -r ./GENERATED_files_dat/$name
  	gcc -DLIN_ALG -DTIME_OFF $TRAPEZOIDAL $Q $DDL $TRI $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDL one_step_trapezoidal_q.c -lm
	  ./$MAIN $TTIME $TSTEP $name;
		fi

		if [ "$FORM" == 2 ];  then 
		echo "Compiling Model with Udwadia Formalism and TRAPEZOIDAL integrator, solving with LIN_ALG library";
    name="state_lin_alg_Trapezoidal_Udwadia_form.dat"; rm -f -r ./GENERATED_files_dat/$name
  	gcc -DLIN_ALG -DTIME_OFF $TRAPEZOIDAL $Q $DDQ $TRI $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Q $DYN_DDQ one_step_trapezoidal_q.c -lm
	  ./$MAIN $TTIME $TSTEP $name;
		fi

		if [ "$FORM" == 3 ];  then 
		echo "Compiling Model with Independent Velocity Formalism and TRAPEZOIDAL integrator, solving with LIN_ALG library";
    name="state_lin_alg_Trapezoidal_Indep_Vel_form.dat"; rm -f -r ./GENERATED_files_dat/$name
  	gcc -DLIN_ALG -DTIME_OFF -DROBUST_SOLVER $TRAPEZOIDAL $Z $TOL $AUX -O2 -o $MAIN $MAIN.c $COMMON $L_A $COORD_Z one_step_trapezoidal_z.c -lm
	  ./$MAIN $TTIME $TSTEP $name;		
    fi
