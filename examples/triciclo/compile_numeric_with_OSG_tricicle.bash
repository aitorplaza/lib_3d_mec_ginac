...
TRICICLE="Geom_Kin_Model_plane.c  Kinematics_Wheels.c  TyreModel.c"
TRICICLE_o="Geom_Kin_Model_plane.o  Kinematics_Wheels.o  TyreModel.o"
###################################################################################################
MAIN=main_numeric_osg_tricicle
echo "";
...
ALFA=1.0e+12
	...
	if [ "$INTEG" == 1 ];  then
		...
		gcc -DEULER -DCOORD_DEP -DDDL -DTRI $TOL -O2 -c read_config_file.c $COMMON $COORD_Q $L_A  $DYN_DDL $TRICICLE one_step_euler_q.c
		g++ -c $OSG
		gcc  -DEULER -DCOORD_DEP -DDDL -DTRI $TOL -DLIN_ALG -O2 -o $MAIN $MAIN.cc $COMMON_o $COORD_Q_o $DYN_DDL_o $L_A_o $TRICICLE_o $OSG_o read_config_file.c one_step_euler_q.o -lOpenThreads -losg -losgDB -losgGA -losgUtil -losgViewer -losgText -lm -lblas -llapack -ansi -funroll-loops
		fi
...
rm -f TyreModel.c
rm -f TyreModel.h
rm -f direct_dynamics_lin_alg_tricicle.c 
rm -f main_numeric_osg_tricicle.cc
